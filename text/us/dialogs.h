// Parameters: dialog enum ID, (unused), lines per box, left offset, width

#ifdef VERSION_EU
#define COMRADES "friends"
#define PLASTERED "splattered"
#define SCAM_ME "cheat!\n"
#define SCRAM "get lost"
#define YOU_CANT_SWIM_IN_IT "Its too heavy to swim\nwith."
#define GIVE_UP "give up"
#else
#define COMRADES "comrades"
#define PLASTERED "plastered"
#define SCAM_ME "scam\nME. "
#define SCRAM "scram--"
#define YOU_CANT_SWIM_IN_IT "You can't swim in it."
#define GIVE_UP "give"
#endif

DEFINE_DIALOG(DIALOG_000, 1, 1, 95, 180, _("\
Maison rocher.\n\
Maison solide !\n\
Personne entrer."))

DEFINE_DIALOG(DIALOG_001, 1, 3, 130-42, 220, _("\
Déplace le stick analogique\n\
pour sélectionner un badge.\n\
[A] pour confirmer l'achat."))

DEFINE_DIALOG(DIALOG_002, 1, 2, 95, 180, _("\
Je chef.\n\
Aime village.\n\
Jolie vue. Maison\n\
très confort.\n\
Tu visiter\n\
quand vouloir !"))

DEFINE_DIALOG(DIALOG_003, 1, 2, 95, 180, _("\
Bonjour. Bienvenue village.\n\
Tribu content toi voir.\n\
Avoir problème.\n\
Aider nous ?\n\
Dinosaure méchant.\n\
Très problème !\n\
Poule aussi !\n\
Jeter oeuf sur maison !\n\
Chasse dino.\n\
Chasse poule.\n\
Te donner joli\n\
pierre brille."))

DEFINE_DIALOG(DIALOG_004, 1, 3, 95, 180, _("\
Arrrr ! Mon plus précieux\n\
trésor est passé par-dessus\n\
bord...\n\
Est-ce que t'aurais la bonté\n\
d'aller m'le récupérer dans\n\
les glaciers, moussaillon ?"))

DEFINE_DIALOG(DIALOG_005, 1, 3, 30, 180, _("\
Hey, Mario! Is it true\n\
that you beat the Big\n\
Bob-omb? Cool!\n\
You must be strong. And\n\
pretty fast. So, how fast\n\
are you, anyway?\n\
Fast enough to beat me...\n\
Koopa the Quick? I don't\n\
think so. Just try me.\n\
How about a race to the\n\
mountaintop, where the\n\
Big Bob-omb was?\n\
Whaddya say? When I say\n\
『Go,』 let the race begin!\n\
\n\
Ready....\n\
\n\
//Go!////Don't Go"))

DEFINE_DIALOG(DIALOG_006, 1, 3, 95, 180, _("\
Yar har har ! Ce calice,\n\
c'est le joyau de tout\n\
mon butin !\n\
Comment te remercier, p'tit\n\
gars ? Une bonne bouteille ?\n\
Cent mille pièces d'or ?\n\
Arrrr, je sais ! Attrape\n\
ce trésor. Et puisses-tu\n\
avoir le vent en poupe !"))

DEFINE_DIALOG(DIALOG_007, 1, 3, 95, 180, _("\
Ouste ! T'as déjà ton\n\
étoile, alors laisse ce\n\
vieux cap'taine tranquille."))

DEFINE_DIALOG(DIALOG_008, 1, 3, 95, 180, _("\
Escalader rocher.\n\
Content ! Mais\n\
maintenant coincé.\n\
Toi. Montrer moi\n\
comment descendre !\n\
\n\
Sauter. Courage.\n\
Avant toucher sol,\n\
coup de pied.\n\
Je regarder !"))

DEFINE_DIALOG(DIALOG_009, 1, 5, 30, 200, _("\
Long time, no see! Wow,\n\
have you gotten fast!\n\
Have you been training\n\
on the sly, or is it the\n\
power of the Stars?\n\
I've been feeling down\n\
about losing the last\n\
race. This is my home\n\
course--how about a\n\
rematch?\n\
The goal is in\n\
Windswept Valley.\n\
Ready?\n\
\n\
//Go//// Don't Go"))

DEFINE_DIALOG(DIALOG_010, 1, 4, 30, 200, _("\
You've stepped on the\n\
Wing Cap Switch. Wearing\n\
the Wing Cap, you can\n\
soar through the sky.\n\
Now Wing Caps will pop\n\
out of all the red blocks\n\
you find.\n\
\n\
Would you like to Save?\n\
\n\
//Yes////No"))

DEFINE_DIALOG(DIALOG_011, 1, 4, 30, 200, _("\
You've just stepped on\n\
the Metal Cap Switch!\n\
The Metal Cap makes\n\
Mario invincible.\n\
Now Metal Caps will\n\
pop out of all of the\n\
green blocks you find.\n\
\n\
Would you like to Save?\n\
\n\
//Yes////No"))

DEFINE_DIALOG(DIALOG_012, 1, 4, 30, 200, _("\
You've just stepped on\n\
the Vanish Cap Switch.\n\
The Vanish Cap makes\n\
Mario disappear.\n\
Now Vanish Caps will pop\n\
from all of the blue\n\
blocks you find.\n\
\n\
Would you like to Save?\n\
\n\
//Yes////No"))

DEFINE_DIALOG(DIALOG_013, 1, 5, 30, 200, _("\
You've collected 100\n\
coins! Mario gains more\n\
power from the castle.\n\
Do you want to Save?\n\
//Yes////No"))

DEFINE_DIALOG(DIALOG_014, 1, 4, 30, 200, _("\
Wow! Another Power Star!\n\
Mario gains more courage\n\
from the power of the\n\
castle.\n\
Do you want to Save?\n\
\n\
//You Bet//Not Now"))

DEFINE_DIALOG(DIALOG_015, 1, 4, 30, 200, _("\
You've just stepped on\n\
the Vanish Cap Switch.\n\
The Vanish Cap makes\n\
Mario disappear.\n\
Now Vanish Caps will pop\n\
from all of the blue\n\
blocks you find.\n\
\n\
Would you like to Save?\n\
\n\
//Yes////No"))

DEFINE_DIALOG(DIALOG_016, 1, 3, 95, 180, _("\
Le cap'taine a le mal de\n\
mer, on dirait. Il s'est \n\
enfermé dans ses quartiers.\n\
La clé est dans la cale.\n\
J'te dis ça comme ça,\n\
matelot."))

DEFINE_DIALOG(DIALOG_017, 1, 4, 30, 200, _("\
I'm the Big Bob-omb, lord\n\
of all blasting matter,\n\
king of ka-booms the\n\
world over!\n\
How dare you scale my\n\
mountain? By what right\n\
do you set foot on my\n\
imperial mountaintop?\n\
You may have eluded my\n\
guards, but you'll never\n\
escape my grasp...\n\
\n\
...and you'll never take\n\
away my Power Star. I\n\
hereby challenge you,\n\
Mario!\n\
If you want the Star I\n\
hold, you must prove\n\
yourself in battle.\n\
\n\
Can you pick me up from\n\
the back and hurl me to\n\
this royal turf? I think\n\
that you cannot!"))

DEFINE_DIALOG(DIALOG_018, 1, 4, 95, 180, _("\
Arrr ! Bienvenue à bord,\n\
moussaillon ! Des boo\n\
de malheur ont envahi\n\
not'navire.\n\
On sait pas d'où y\n\
viennent, mais il va\n\
falloir qu'ils partent."))

DEFINE_DIALOG(DIALOG_019, 1, 3, 95, 180, _("\
Pas trop le vertige,\n\
moussaillon ?\n\
\n\
Un rafiot maléfique nous\n\
hante, nous et notre navire,\n\
depuis la nuit des temps.\n\
Si tu l'oses, libère le fléau\n\
et mets fin à sa tyrannie\n\
une bonne fois pour toutes !"))

DEFINE_DIALOG(DIALOG_020, 1, 6, 95, 150, _("\
Bienvenue dans la salle de jeu !\n\
Ici, tu peux acheter et jouer à\n\
des mini-jeux, ou voir tes scores."))

DEFINE_DIALOG(DIALOG_021, 1, 5, 95, 180, _("\
Acheter [%] pour [%]$?\n\
(Solde actuel : [%]$)"))

DEFINE_DIALOG(DIALOG_022, 1, 2, 95, 180, _("\
Cette porte ne peut être ouverte que par\n\
la puissance de [%] étoiles dorées.\n\
Il t'en faut encore [%]."))

DEFINE_DIALOG(DIALOG_023, 1, 3, 95, 180, _("\
Tu as acheté [%] !"))

DEFINE_DIALOG(DIALOG_024, 1, 5, 95, 180, _("\
Nouveau record pour [%] : [%] !"))

DEFINE_DIALOG(DIALOG_025, 1, 4, 95, 180, _("\
Cette porte ne peut être ouverte que par\n\
la puissance de [%] graines cosmiques.\n\
Il t'en faut encore [%]."))

DEFINE_DIALOG(DIALOG_026, 1, 4, 95, 180, _("\
Bienvenue dans le vestiaire !\n\
Ici, tu peux voir et équiper tes costumes."))

DEFINE_DIALOG(DIALOG_027, 1, 4, 95, 180, _("\
Bienvenue dans la salle de musique !\n\
Ici, tu peux écouter la bande-son du jeu."))

DEFINE_DIALOG(DIALOG_028, 1, 4, 95, 180, _("\
On dirait que cette porte a été verrouillée par\n\
le Présentateur. Peut-être faudra-t-il \n\
revenir après un évènement important ?"))

DEFINE_DIALOG(DIALOG_029, 1, 3, 95, 180, _("\
Tu nous as sauvés ! T'es not'\n\
nouveau shérif, maintenant.\n\
Allez, prends ton badge !"))

DEFINE_DIALOG(DIALOG_030, 1, 4, 95, 180, _("\
Ah, un visiteur !\n\
Bienvenue dans ma boutique\n\
SUPER SECRÈTE !!\n\
\n\
Si je me cache, c'est\n\
parce que sinon j'aurais\n\
trop de clients ! Mes\n\
prix sont imbattables !\n\
Viens me parler si tu\n\
veux m'acheter quelque\n\
chose."))

DEFINE_DIALOG(DIALOG_031, 1, 5, 30, 200, _("\
No way! You beat me...\n\
again!! And I just spent\n\
my entire savings on\n\
these new Koopa\n\
Mach 1 Sprint shoes!\n\
Here, I guess I have to\n\
hand over this Star to\n\
the winner of the race.\n\
Congrats, Mario!"))

DEFINE_DIALOG(DIALOG_032, 1, 3, 95, 180, _("\
Après avoir acheté un\n\
badge, appuie sur START\n\
pour ouvrir le menu pause.\n\
Ensuite, appuie sur [L]\n\
et [R] pour aller dans\n\
l'onglet BADGES, puis\n\
sélectionne un badge\n\
avec le stick et appuie\n\
sur [A] pour l'équiper.\n\
Tu ne peux équiper qu'un\n\
certain nombre de badges\n\
à la fois, selon ton PB.\n\
Pour augmenter ton PB et\n\
équiper plus de badges,\n\
tu dois monter de niveau."))

DEFINE_DIALOG(DIALOG_033, 1, 7, 95, 180, _("\
Les courants sont forts,\n\
par ici. Je peux lever\n\
le pont pour 10 pièces !\n\
Eh oui, je sais, c'est\n\
cher, mais c'est comme ça.\n\
Allez, appuie sur [A]\n\
pour envoyer l'argent."))

/*
DEFINE_DIALOG(DIALOG_034, 1, 4, 95, 180, _("\
The pure and holy\n\
water of the fountain\n\
will cleanse one's\n\
woes and sorrows."))
*/

//VALVE PLAYTESTER MOMENT:
//
DEFINE_DIALOG(DIALOG_034, 1, 2, 95, 180, _("\
Le raccourci secret\n\
de Bowser"))


DEFINE_DIALOG(DIALOG_035, 1, 2, 95, 180, _("\
Salle de recherche\n\
préhistorique"))

DEFINE_DIALOG(DIALOG_036, 1, 4, 95, 180, _("\
Les autres voyous, tout\n\
ce qu'ils veulent, c'est\n\
te mettre des coups d'boule.\n\
Bah pas moi.\n\
Moi, j'suis là pour\n\
me remplir les poches,\n\
tu captes ?\n\
\n\
Tu feras gaffe,\n\
la lave, ça brûle.\n\
Heureusement pour toi,\n\
j'ai ce qu'il te faut."))

DEFINE_DIALOG(DIALOG_037, 1, 5, 95, 180, _("\
Dans les boîtes jaunes se\n\
trouve la carapace verte.\n\
Si tu grimpes dessus, tu\n\
pourras surfer sur la lave\n\
sans risque de brûlure.\n\
Pour rendre ces boîtes\n\
solides, il faut trouver\n\
l'interrupteur jaune.\n\
Son emplacement est un\n\
secret bien gardé.\n\
La légende dit ainsi :\n\
Dans la salle ornée d'un 2,\n\
Sur un meuble en bois ciré,\n\
Une attaque par les cieux\n\
Révèlera la voie cachée."))

DEFINE_DIALOG(DIALOG_038, 1, 4, 95, 180, _("\
Que d'étoiles, ma parole !\n\
Mais attention aux\n\
contrefaçons ! Laquelle\n\
choisiras-tu ?"))

DEFINE_DIALOG(DIALOG_039, 1, 4, 95, 180, _("\
Les tyroliennes, c'est\n\
l'éclate ! Mais avant de\n\
te lancer, il va te falloir\n\
de quoi t'accrocher.\n\
Un bout de tuyau, par\n\
exemple. Peut-être qu'il\n\
y en a un dans le coin."))

DEFINE_DIALOG(DIALOG_040, 1, 5, 95, 180, _("\
Bonjour, jeune homme !\n\
Maintiens [A] pour\n\
transférer les pièces\n\
de ce niveau vers ton\n\
porte-monnaie global."))

DEFINE_DIALOG(DIALOG_041, 1, 5, 95, 180, _("\
saaaaalut ¤^¤^\n\
sa va ?? moi sa va pas...\n\
le proprio a mit notre\n\
fromage sur des piege\n\
mortel cette fraude !!\n\
tu pense que tu pourrait\n\
nous aider ?? g trouver\n\
une étoile je te la \n\
donne si tu me ramene\n\
le fromage !!\n\
ah je t'ouvrirait les\n\
porte aussi ¤^¤^ hihi"))

DEFINE_DIALOG(DIALOG_042, 1, 4, 95, 180, _("\
le proprio a laisser\n\
trainer ses vieille\n\
chaussette qui pue...\n\
remets les chaussette\n\
dans le panier a linge\n\
pour nous stp ¤^¤^\n\
on peut pas jouer a mario\n\
tranquille avec cette\n\
giga odeur de pied !!"))

DEFINE_DIALOG(DIALOG_043, 1, 2, 95, 180, _("\
euhhhh la maison est\n\
en train de bruler la ¤^¤^\n\
faudrait genre eteindre\n\
le four ou jsp quoi ??"))

DEFINE_DIALOG(DIALOG_044, 1, 3, 95, 180, _("\
t'as sauver la maison\n\
t vraiment le goat !!!\n\
merci chef !!"))

DEFINE_DIALOG(DIALOG_045, 1, 2, 95, 180, _("\
chef g trouver des\n\
anneaux jaune !\n\
tu veux pas genre\n\
sauter dedans jsp ??"))

DEFINE_DIALOG(DIALOG_046, 1, 3, 95, 180, _("\
coucou :3\n\
merci d avoir trouver\n\
le fromage pr nous !\n\
on va bien se regaler\n\
mtn ¤^¤^ miam\n\
\n\
mes mutu et moi on ta\n\
ouvert les autre porte\n\
de la maison aussi !!"))

DEFINE_DIALOG(DIALOG_047, 1, 2, 95, 180, _("\
Hi! I'll prepare the\n\
cannon for you!"))

DEFINE_DIALOG(DIALOG_048, 1, 3, 95, 180, _("\
mec c horrible...\n\
ya un gros drama en \n\
ce moment...\n\
la maison elle est\n\
EN FEU mec !!!\n\
c une dinguerie...\n\
il faut trouver dou\n\
sa vient pcq sinon\n\
on est finito !!!"))

DEFINE_DIALOG(DIALOG_049, 1, 3, 95, 180, _("\
le sous-sol me fait\n\
bcp trop peur...\n\
\n\
enft c juste une pièce\n\
normale mais ya un \n\
monstre dedans\n\
du coup fait gaffe..."))

DEFINE_DIALOG(DIALOG_050, 1, 4, 95, 180, _("\
Bienvenue   à   rétro\n\
land !   Il   y   a\n\
plein   de   choses\n\
à   faire   ici !\n\
Appuie   sur   [R]\n\
pour   dézoomer\n\
la   caméra !"))

DEFINE_DIALOG(DIALOG_051, 1, 3, 95, 180, _("\
Attention   au\n\
labyrinthe   de\n\
tuyaux !\n\
Il   est   plein\n\
de   plantes\n\
piranha !"))

DEFINE_DIALOG(DIALOG_052, 1, 4, 95, 180, _("\
La   vue   d'ici\n\
est   magnifique !\n\
On   voit   bien\n\
les   pixels ."))

DEFINE_DIALOG(DIALOG_053, 1, 2, 95, 180, _("\
Voici   le   mont\n\
Bill   Balle.\n\
Les   Bill   Balle\n\
sont   dangereux.\n\
Fais   attention\n\
à   toi !"))

DEFINE_DIALOG(DIALOG_054, 1, 4, 95, 180, _("\
Je   suis   fatigué\n\
et   complètement\n\
perdu   . . .\n\
Où   suis - je ?"))

DEFINE_DIALOG(DIALOG_055, 1, 4, 30, 200, _("\
Hey-ey, Mario, buddy,\n\
howzit goin'? Step right\n\
up. You look like a fast\n\
sleddin' kind of guy.\n\
I know speed when I see\n\
it, yes siree--I'm the\n\
world champion sledder,\n\
you know. Whaddya say?\n\
How about a race?\n\
Ready...\n\
\n\
//Go//// Don't Go"))

DEFINE_DIALOG(DIALOG_056, 1, 5, 95, 180, _("\
vVVVVVVVVVVV\n\
C'est moi qui SSSSuis\n\
en CHHHHarge de garder\n\
Cette maison propre.\n\
\n\
Ces rats m'ont laissé\n\
un SSSSSSSacré bazar !\n\
C'est VVVVVVVVraiment\n\
insupportable ! Je vais\n\
tous VVVVVous aspirer !!"))

DEFINE_DIALOG(DIALOG_057, 1, 3, 95, 180, _("\
Mario !\n\
Comme   on   se\n\
retrouve   enfin .\n\
Tu   m'as   l'air\n\
différent   cette\n\
fois - ci .\n\
Je   protégerai\n\
cette   graine\n\
cosmique . . .\n\
A v e c . . .\n\
M a . . .\n\
V I E ! ! !"))

DEFINE_DIALOG(DIALOG_058, 1, 2, 95, 180, _("\
Tu places ton oeil sur\n\
le télescope, et tu vois...\n\
Rien du tout. On dirait\n\
qu'il manque la lentille..."))

DEFINE_DIALOG(DIALOG_059, 1, 2, 95, 180, _("\
Tu fixes la lentille\n\
sur le télescope..."))

DEFINE_DIALOG(DIALOG_060, 1, 4, 95, 180, _("\
Alors que tu regardes\n\
le ciel à travers le\n\
télescope, tu sens ton\n\
âme s'envoler..."))

DEFINE_DIALOG(DIALOG_061, 1, 5, 95, 180, _("\
Yar har har !\n\
C'est l'prospecteur !\n\
Ça fait longtemps qu'j'ai\n\
péri dans cette mine, mais\n\
mon âme est toujours là.\n\
J'te propose un marché.\n\
J'veux te voir sauter à \n\
la corde de flammes 25\n\
fois, et j'te filerai\n\
mon étoile ! Har har !"))

DEFINE_DIALOG(DIALOG_062, 1, 3, 95, 180, _("\
Bienvenue au Canyon\n\
Cowboy, part'naire !\n\
Yiii haa !\n\
R'garde bien où tu mets les\n\
pieds, p'tit gars. On est\n\
sacrément haut, ici !"))

DEFINE_DIALOG(DIALOG_063, 1, 4, 95, 180, _("\
Tu vois c'te balle ? J'suis\n\
sûr qu't'as la place pour\n\
mettre ta tête dedans !\n\
\n\
Maintiens [A] en l'air pour\n\
filer en ligne droite !\n\
C'est pas un vol très long,\n\
alors sois prudent !"))

DEFINE_DIALOG(DIALOG_064, 1, 3, 95, 180, _("\
Yiiii haa ! Alors part'naire,\n\
qu'est-ce que tu dis\n\
d'cette vue ?"))

DEFINE_DIALOG(DIALOG_065, 1, 4, 95, 180, _("\
Halte là, part'naire !\n\
Ce chariot mène tout droit\n\
à la vieille mine du\n\
prospecteur.\n\
Là où y'a d'l'or, y'a des\n\
risques. Alors gardes tes\n\
yeux bien grand ouverts.\n\
\n\
On raconte que l'âme du\n\
vieux prospecteur hante\n\
encore les ténèbres de\n\
la grotte."))

DEFINE_DIALOG(DIALOG_066, 1, 2, 95, 180, _("\
Des hors-la-loi\n\
envahissent la ville !"))

DEFINE_DIALOG(DIALOG_067, 1, 4, 95, 180, _("\
Qui est ce mécréant,\n\
agenouillé devant ma\n\
royale personne ?\n\
\n\
Me supplie-t-il de lui\n\
conférer mon étoile, et\n\
de lui donner accès aux\n\
bottes-fusées ?\n\
S'il en est ainsi, il\n\
devra d'abord m'affronter\n\
en duel, afin de me prouver\n\
la valeur de son âme.\n\
En garde !"))

DEFINE_DIALOG(DIALOG_068, 1, 3, 95, 180, _("\
Homme de chair, tu as\n\
prouvé ta force.\n\
Prends mon étoile. \n\
J'ai également fait lever\n\
la barrière qui scellait\n\
les bottes-fusée."))

DEFINE_DIALOG(DIALOG_069, 1, 3, 95, 180, _("\
Homme de chair, je dois\n\
te confier une mission\n\
d'importance capitale.\n\
Ma femme est victime\n\
d'une maladie qui la\n\
rend incontrôlable.\n\
Pitié, homme de chair.\n\
Trouve une solution pour\n\
la guérir de ce mal."))

DEFINE_DIALOG(DIALOG_070, 1, 3, 95, 180, _("\
Oh, Mario...\n\
Vous arrivez\n\
juste à temps...\n\
Les ténèbres...\n\
Elles corrompent mon\n\
coeur et mon âme...\n\
Je vous en prie,\n\
pardonnez-moi pour ce\n\
que je vais faire..."))

DEFINE_DIALOG(DIALOG_071, 1, 3, 95, 180, _("\
...\n\
\n\
Aaaahhhh...\n\
Je vous remercie, Mario.\n\
Je sens la force maléfique\n\
enfin quitter mon corps.\n\
Le peuple Thwomp vous doit\n\
la reconnaissance éternelle\n\
pour vos actes de bravoure.\n\
Vous avez extrait de\n\
ma personne cette...\n\
étoile de vide.\n\
Je la veux hors de ma vue.\n\
Prenez-la, emportez-la.\n\
Qu'elle disparaisse."))

DEFINE_DIALOG(DIALOG_072, 1, 5, 95, 180, _("\
Par décret du Roi Thwomp,\n\
tout appareil équipé de\n\
fusées est strictement\n\
interdit dans l'enceinte\n\
de ce palais royal !"))

DEFINE_DIALOG(DIALOG_073, 1, 5, 95, 180, _("\
Par décret du Roi Thwomp,\n\
tout civil doit obtenir\n\
la permission d'accéder\n\
à l'interrupteur des\n\
bottes-fusée !"))

DEFINE_DIALOG(DIALOG_074, 1, 3, 95, 180, _("\
Par décret du roi Thwomp,\n\
tout civil qui réussit à\n\
atteindre l'interrupteur\n\
est autorisé à lever la\n\
barrière et rencontrer\n\
Sa Majesté le Roi !"))

DEFINE_DIALOG(DIALOG_075, 1, 3, 95, 180, _("\
Voici le trou royal qui mène\n\
à la salle du trône royal.\n\
\n\
Soyez prêt à rencontrer\n\
notre pondéreux et\n\
solide Roi !"))

DEFINE_DIALOG(DIALOG_076, 1, 2, 95, 180, _("\
Un civil a osé me\n\
traiter de POULET !\n\
Personne n'a le droit de\n\
me traiter de poulet !!\n\
J'ressemble pas à\n\
un poulet, hein ?"))

DEFINE_DIALOG(DIALOG_077, 1, 6, 95, 180, _("\
Bienvenue au Virtuaplex,\n\
%UTILISATEUR% !\n\
Cet endroit est un\n\
simulacre de la réalité.\n\
\n\
\n\
C'est un univers libre\n\
de toute interférence\n\
extérieure, fabriqué avec\n\
des DDD-0945-DDonnées de\n\
l'extérieur et la puissance \n\
des étoiles.\n\
À notre droite se trouvent\n\
les champs d'octets, et à\n\
notre gauche, la station de\n\
tramway. Le grand couloir\n\
mène directement au bureau\n\
de %DONNÉES SUPPRIMÉES%."))

DEFINE_DIALOG(DIALOG_078, 1, 5, 95, 180, _("\
Hé ! Cet ascenseur mène\n\
au bureau du Directeur.\n\
Personne n'y est permis,\n\
à la seule exception\n\
des administrateurs."))

DEFINE_DIALOG(DIALOG_079, 1, 5, 95, 180, _("\
Hey ho !\n\
Les tyroliennes, c'est\n\
amusant, mais sans le bon\n\
outil pour glisser, tu te\n\
brûleras les mains.\n\
Un bout de tuyau ferait\n\
parfaitement l'affaire.\n\
Et accroche-toi bien,\n\
c'est une sacrée chute\n\
depuis les toits !"))

DEFINE_DIALOG(DIALOG_080, 1, 6, 95, 180, _("\
Bonjour !\n\
Nous sommes des IA\n\
appelées %ÊTRES NÉANT%\n\
générées par le Virtuaplex\n\
dans le but de construire\n\
cette merveilleuse ville.\n\
Si tu remarques un\n\
comportement anormal de\n\
la part d'un être néant,\n\
informes-en les autorités\n\
du Virtuaplex."))

DEFINE_DIALOG(DIALOG_081, 1, 4, 30, 200, _("\
La barrière est levée.\n\
Tu peux passer !"))

DEFINE_DIALOG(DIALOG_082, 1, 4, 30, 200, _("\
Le sol tremble sous tes pieds..."))

DEFINE_DIALOG(DIALOG_083, 1, 6, 30, 200, _("\
Le Pokey Ancestral s'éveille !"))

DEFINE_DIALOG(DIALOG_084, 1, 3, 30, 200, _("\
Tu as trouvé une bourse. Ton nombre de\n\
pièces maximal est passé de [%] à [%] !\n\
Elle contenait également 50 pièces."))

DEFINE_DIALOG(DIALOG_085, 1, 5, 30, 200, _("\
Tu as trouvé le costume de [%] !\n\
Veux-tu le revêtir ?"))

DEFINE_DIALOG(DIALOG_086, 1, 3, 30, 200, _("\
Tu as enclenché l'interrupteur violet.\n\
Ouvrir une boîte violette te donnera\n\
désormais une paire de Bottes-Fusée !"))

DEFINE_DIALOG(DIALOG_087, 1, 4, 30, 200, _("\
Tu as enclenché l'interrupteur noir.\n\
Un frisson parcourt ton corps entier.\n\
Le sceau des boîtes de Pandore a été brisé."))

DEFINE_DIALOG(DIALOG_088, 1, 5, 30, 200, _("\
Tu as enclenché l'interrupteur cyan.\n\
Ouvrir une boîte cyan te donnera\n\
désormais une casquette d'invétal !"))

DEFINE_DIALOG(DIALOG_089, 1, 5, 95, 180, _("\
Tu as enclenché l'interrupteur jaune.\n\
Ouvrir une boîte jaune te donnera\n\
désormais une carapace de Koopa !"))

DEFINE_DIALOG(DIALOG_090, 1, 3, 95, 180, _("\
Prends garde à cette\n\
abîme. Sa profondeur ne\n\
peut pas être mesurée.\n\
Notre prédiction la\n\
plus récente l'estime à\n\
DONNÉES CORROMPUES"))

DEFINE_DIALOG(DIALOG_091, 2, 2, 30, 200, _("\
''Salutations, Superstar ;\n\
Savais-tu que ton ennemi juré se terrait dans\n\
le château en ruines depuis tout ce temps ?"))

DEFINE_DIALOG(DIALOG_092, 1, 5, 30, 200, _("\
Je te supplie de le vaincre, une fois\n\
pour toutes; pour le bien du royaume.\n\
-Ton ami, Le Présentateur''"))

DEFINE_DIALOG(DIALOG_093, 1, 2, 95, 180, _("\
Raccourci pour speedrunnners\n\
\n\
Ne pas utiliser pendant\n\
une découverte du jeu."))

//do not add anything to this, used for showrunner start cutscene
DEFINE_DIALOG(DIALOG_094, 1, 4, 30, 200, _("\
..."))

DEFINE_DIALOG(DIALOG_095, 1, 4, 30, 200, _("\
Il semblerait qu'il patiente et décuple\n\
sa puissance dans une pièce dissimulée\n\
derrière la vitre d'un miroir maudit."))

DEFINE_DIALOG(DIALOG_096, 1, 4, 30, 200, _("\
Mesdames et messieurs, ni l'un ni l'autre ou entre\n\
les deux, préparez-vous car notre fabuleuse émission\n\
est sur le point de commencer !"))

DEFINE_DIALOG(DIALOG_097, 1, 5, 30, 200, _("\
Animée par votre présentateur préféré,\n\
moi ! Le Présentateur !"))

DEFINE_DIALOG(DIALOG_098, 1, 2, 95, 180, _("\
Mais assez parlé de moi. Bien que je brille de mille\n\
feux, une autre étoile scintille aujourd'hui sur\n\
scène avec nous. Lui, qui a combattu et triomphé"))

DEFINE_DIALOG(DIALOG_099, 1, 5, 95, 180, _("\
du roi des Koopas dans ce château même,\n\
la tête d'affiche du Royaume Champignon,\n\
faites du bruit pour Mario Mario !"))

DEFINE_DIALOG(DIALOG_100, 1, 3, 95, 180, _("\
Notre invité d'honneur est venu aujourd'hui pour\n\
terrasser son ennemi juré. Je doute que le\n\
grand Bowser ait besoin d'être présenté..."))

DEFINE_DIALOG(DIALOG_101, 1, 3, 95, 180, _("\
Ne ratez pas cette ultime confrontation \n\
sur vos écrans de télévision !"))

DEFINE_DIALOG(DIALOG_102, 1, 5, 30, 200, _("\
Cachés derrière un miroir magique, des mondes\n\
prennent vie grâce au pouvoir des étoiles; ces\n\
univers devaient être l'atout caché de Bowser"))

DEFINE_DIALOG(DIALOG_103, 1, 4, 95, 180, _("\
dans l'éventualité qu'il échoue. Vous pensez \n\
qu'il ne voulait pas prendre de risque, ou\n\
bien il savait qu'il allait y rester ?"))

DEFINE_DIALOG(DIALOG_104, 1, 5, 30, 200, _("\
Sachez que le studio a pris la liberté\n\
d'utiliser ses propres étoiles pour \n\
façonner les mondes à sa manière..."))

DEFINE_DIALOG(DIALOG_105, 1, 3, 95, 180, _("\
Afin d'offrir la meilleure des expériences\n\
à notre jeune candidat, bien évidemment..."))

DEFINE_DIALOG(DIALOG_106, 1, 2, 95, 180, _("\
Tant de menaces et de merveilles l'attendent\n\
derrière ce miroir maudit, alors gardez les\n\
yeux rivés sur le spectacle du millénaire !"))

DEFINE_DIALOG(DIALOG_107, 1, 4, 95, 180, _("\
Mario ! Ça fait un bail,\n\
depuis la dernière fois !\n\
Quelle chance de te\n\
croiser ici !\n\
Après que tu m'as battu sur\n\
l'Île Grand-Petit, j'ai pris\n\
ma retraite et lancé ma\n\
propre marque de badges.\n\
Je m'en sors plutôt bien,\n\
les clients affluent et\n\
les badges se vendent\n\
comme des petits pains !\n\
Vas-y, jette un oeil\n\
à ma marchandise !\n\
Tu trouveras peut-être\n\
ton bonheur."))

DEFINE_DIALOG(DIALOG_108, 1, 2, 95, 180, _("\
Le roi des Koopas t'attend.\n\
Et toi, qu'est-ce que tu attends,\n\
superstar ☆ ?"))

DEFINE_DIALOG(DIALOG_109, 1, 5, 95, 180, _("\
...\n\
Prends garde à la tour\n\
radio. Elle atomise toute\n\
forme de vie qui entre dans\n\
sa ligne de mire."))

DEFINE_DIALOG(DIALOG_110, 1, 5, 95, 180, _("\
...\n\
Quand tu es venu sur notre\n\
planète et que tu as\n\
détruit l'étoile obscure,\n\
nous avons eu la paix.\n\
Du moins, jusqu'à ce que\n\
la lune parasite vienne\n\
ici à son tour.\n\
\n\
\n\
Avec elle est descendu\n\
un prophète. Et tous\n\
le regardèrent avec\n\
fascination.\n\
\n\
C'est à ce moment que\n\
la fin de notre monde\n\
a commencé."))

DEFINE_DIALOG(DIALOG_111, 1, 5, 95, 180, _("\
Plus tu arraches de\n\
Graines Cosmiques à\n\
Agamemnon, plus sa\n\
puissance s'affaiblit.\n\
\n\
Une fois que l'abîme\n\
sera suffisamment\n\
affaibli, tu pourras\n\
sûrement sauter dedans\n\
sans être vaporisé.\n\
Ainsi, pourras-tu\n\
enfin mettre un terme\n\
à ce cycle éternel\n\
de mort et de terreur."))

DEFINE_DIALOG(DIALOG_112, 1, 3, 95, 180, _("\
Nous ne sommes pas\n\
innocents non plus.\n\
\n\
Même avant l'arrivée\n\
d'Agamemnon, nous avions\n\
nos terribles secrets.\n\
Agamemnon nous a forcé\n\
à ouvrir le moulin..."))

DEFINE_DIALOG(DIALOG_113, 1, 4, 95, 180, _("\
ATTENTION ! Le beurre à\n\
popcorn est TRÈS CHAUD.\n\
Merci de faire preuve\n\
de prudence."))

DEFINE_DIALOG(DIALOG_114, 1, 3, 95, 180, _("\
Ce roi pathétique traite\n\
les Whomps comme une espèce\n\
inférieure aux Thwomps.\n\
Il nous force à construire\n\
des tours, des maisons et\n\
des châteaux.\n\
Et pas un salaire !\n\
Pas une récompense !\n\
Même pas un merci !!\n\
Cette fois-ci, tu ne\n\
t'en sortiras pas\n\
indemne.\n\
Et maintenant, j'ai\n\
l'avantage du terrain !"))

DEFINE_DIALOG(DIALOG_115, 1, 4, 95, 180, _("\
Tu n'es qu'un serviteur\n\
du roi. Un misérable\n\
pion, c'est tout !\n\
\n\
Un jour, il t'appelle\n\
son ami, et le lendemain,\n\
tu n'es qu'une pierre\n\
sur son chemin !\n\
Tu verras bien, idiot\n\
de plombier...\n\
Tu verras bien !\n\
\n\
En tout cas, pour toi,\n\
c'est échec et mat.\n\
J'espère que tu sais,\n\
voler, petit pion !"))

DEFINE_DIALOG(DIALOG_116, 1, 5, 95, 180, _("\
Whaaa....Whaaat?\n\
Can it be that a\n\
pipsqueak like you has\n\
defused the Bob-omb\n\
king????\n\
You might be fast enough\n\
to ground me, but you'll\n\
have to pick up the pace\n\
if you want to take King\n\
Bowser by the tail.\n\
Methinks my troops could\n\
learn a lesson from you!\n\
Here is your Star, as I\n\
promised, Mario.\n\
\n\
If you want to see me\n\
again, select this Star\n\
from the menu. For now,\n\
farewell."))

DEFINE_DIALOG(DIALOG_117, 1, 5, 95, 180, _("\
...Halte, virus Terrien.\n\
Tu as rompu la paix entre\n\
les Éternels et Agamemnon,\n\
n'est-ce pas ?\n\
\n\
Peu importe. Ce n'était\n\
qu'un traité d'intégration\n\
entre le prophète et la\n\
planète. La phagocytose\n\
était volontaire.\n\
Les Graines Cosmiques se\n\
grefferont sur leurs corps\n\
sans vie, et eux aussi\n\
deviendront Agamemnon.\n\
\n\
...Tu espérais voir le\n\
prophète de ce monde,\n\
c'est bien ça ?\n\
\n\
\n\
Elle est devenue une\n\
entité de PALAMEDES. \n\
Il aurait été inutile de\n\
la garder ici. Les fidèles\n\
vénèrent son image,\n\
pas sa personne.\n\
Ainsi, moi, Terrien, ai\n\
pris la relève.\n\
\n\
\n\
Le prophète décédé a amené\n\
nombre d'êtres puissants à\n\
Agamemnon et leur a offert\n\
la puissance cosmique.\n\
Tu devais être le suivant.\n\
Mais tu as interrompu le\n\
flux d'attention des\n\
Éternels. De ce fait,\n\
tu dois être supprimé.\n\
Ton monde est Agamemnon."))

DEFINE_DIALOG(DIALOG_118, 1, 3, 95, 180, _("\
...\n\
\n\
\n\
Tout ça,\n\
c'est de sa faute."))

DEFINE_DIALOG(DIALOG_119, 1, 6, 95, 180, _("\
Ce satané porc est sans\n\
arrêt en colère !!\n\
Le seul truc qui peut\n\
l'calmer, c'est un\n\
peu d'musique.\n\
-- Le fermier"))

DEFINE_DIALOG(DIALOG_120, 1, 7, 95, 180, _("\
Conduire une calèche,\n\
c'est facile ! Saute\n\
dessus et maintiens le\n\
stick analogique dans la\n\
direction souhaitée.\n\
Pour descendre, il\n\
suffit de sauter."))

DEFINE_DIALOG(DIALOG_121, 1, 6, 95, 180, _("\
Ah, ces mauvaises herbes !\n\
Elles sont en train d'me\n\
bousiller mon champ.\n\
Je paierai grassement\n\
quiconque m'les arrache !\n\
-- Le fermier"))

DEFINE_DIALOG(DIALOG_122, 1, 5, 95, 180, _("\
Bienvenue à Indigopolis.\n\
C'est ici que sont générés\n\
l'électricité et le WI-FI\n\
pour les habitants de\n\
Festella.\n\
Cet endroit se trouve à des\n\
milliers de kilomètres sous\n\
la ville. Peu de gens savent\n\
que nous existons, mais nos\n\
services sont indispensables."))

DEFINE_DIALOG(DIALOG_123, 1, 4, 95, 180, _("\
Attention à l'acide de\n\
serveur. C'est très utile,\n\
mais aussi très dangereux.\n\
\n\
Tout appareil qui est plongé\n\
dedans voit ses capacités\n\
augmenter de 300%.\n\
\n\
Il nous sert également de\n\
stockage, puisqu'il peut\n\
contenir 1 TB de données\n\
par centimètre cube.\n\
Cependant, dû à son pH\n\
extrêmement bas, il fait\n\
perdre 5 points de vie\n\
aux humains comme toi."))

DEFINE_DIALOG(DIALOG_124, 1, 3, 95, 180, _("\
Caché quelque part dans\n\
cette ville se trouve\n\
l'interrupteur cyan.\n\
Il permet de débloquer la\n\
casquette d'Invétal, une\n\
casquette expérimentale\n\
qui combine les effets\n\
d'invisibilité et de métal."))


DEFINE_DIALOG(DIALOG_125, 1, 5, 95, 180, _("\
Cette zone était autrefois\n\
densément peuplée. Mais\n\
avec le temps, l'acide a\n\
dégradé la qualité de l'air.\n\
\n\
Il y avait des voitures,\n\
des lampadaires, des bancs,\n\
des posters, et beaucoup\n\
d'autres objets utiles\n\
aux êtres humains.\n\
Quand ceux-ci sont partis,\n\
nous avons détruit tous\n\
ces artéfacts pour en\n\
extraire les matériaux.\n\
\n\
Les bâtiments ne servent\n\
plus qu'à héberger les\n\
serveurs, à l'exception\n\
de la boutique et de\n\
la banque.\n\
Il n'y a plus que nous pour\n\
entretenir l'équipement.\n\
La majorité des habitants\n\
a déménagé à Festella."))

DEFINE_DIALOG(DIALOG_126, 2, 6, 95, 180, _("\
Problème !! Problème !!\n\
Un certain ''présentateur''\n\
s'est imposé dans notre\n\
bâtiment gouvernemental.\n\
Il va falloir corriger\n\
ce bug, et vite !"))

DEFINE_DIALOG(DIALOG_127, 3, 3, 95, 180, _("\
C'est quoi, ce contour\n\
de pièce en pointillés ?\n\
Une pièce Yoshi, bien sûr !\n\
Tu ne peux les ramasser\n\
que si tu es sur le dos\n\
d'un dinosaure vert."))

DEFINE_DIALOG(DIALOG_128, 1, 4, 95, 180, _("\
You must fight with\n\
honor! It is against the\n\
royal rules to throw the\n\
king out of the ring!"))

DEFINE_DIALOG(DIALOG_129, 1, 5, 30, 200, _("\
Il te faut une clé pour ouvrir cette porte. Capisce ?"))

DEFINE_DIALOG(DIALOG_130, 2, 6, 95, 180, _("\
Une des Graines Cosmiques\n\
confiées aux Éternels\n\
semble avoir disparu.\n\
Elles sont pourtant si\n\
étroitement gardées !\n\
Serait-ce grâce à toi ?\n\
Je fais partie des Rebelles.\n\
Nous ne sommes plus qu'une\n\
douzaine. Nous descendons\n\
des marins qui autrefois\n\
ont dompté ces mers, et\n\
nous renions Agamemnon.\n\
L'Empire est naïf de \n\
croire pouvoir atteindre\n\
l'immortalité...\n\
Ils sont stagnants, leurs\n\
esprits peu à peu rongés\n\
par la berceuse lunaire.\n\
Nous pouvons accélérer\n\
ce processus.\n\
\n\
\n\
\n\
\n\
Tu as trouvé une arme.\n\
Très bien. Si tu t'occupes\n\
de détruire leurs pousses\n\
de hautecanne, je te\n\
donnerai une des Graines\n\
Cosmiques que j'ai volées."))

DEFINE_DIALOG(DIALOG_131, 2, 6, 95, 180, _("\
Une grande partie de ce\n\
pilier naturel s'est\n\
effondrée. De ce fait,\n\
il est presque impossible\n\
d'avancer davantage.\n\
\n\
Pas que cela ait la moindre\n\
importance, puisque les\n\
Éternels n'ont pas miné\n\
cette roche depuis bien\n\
des millénaires.\n\
\n\
La seule exception fut\n\
lorsqu'un prophète de la\n\
Lune est arrivé au temple\n\
pour s'entraîner et l'a\n\
partiellement détruit en\n\
tournoyant comme un idiot.\n\
En bref, il va falloir\n\
explorer les environs pour\n\
trouver comment atteindre\n\
le haut de la montagne.\n\
Fais bon usage de tes\n\
badges et autres pouvoirs."))

DEFINE_DIALOG(DIALOG_132, 1, 6, 95, 180, _("\
Cette chute de lave est\n\
très dangereuse. Pour la\n\
conquérir, il faudra\n\
combiner sauts latéraux\n\
et attaques rodéo.\n\
\n\
En cas de difficulté,\n\
le badge Chaud Derrière\n\
peut se montrer utile.\n\
Celui-ci nullifie les\n\
dégâts de lave en\n\
échange de points de Mana."))

DEFINE_DIALOG(DIALOG_133, 1, 5, 95, 180, _("\
Mario, je te prie d'accepter\n\
mon éternelle gratitude.\n\
Le royaume t'est redevable.\n\
La Reine et moi te\n\
garderons toujours dans\n\
nos coeurs de pierre."))

DEFINE_DIALOG(DIALOG_134, 1, 1, 95, 180, _("\
Salle de musique"))

DEFINE_DIALOG(DIALOG_135, 1, 1, 95, 180, _("\
Virtuaplex"))

DEFINE_DIALOG(DIALOG_136, 1, 1, 95, 180, _("\
Vestiaire"))

DEFINE_DIALOG(DIALOG_137, 1, 3, 95, 180, _("\
Une   serrure   rouge ?\n\
Il   te   faut   sans\n\
doute   une   clé   rouge   !"))

DEFINE_DIALOG(DIALOG_138, 1, 3, 95, 180, _("\
mec\n\
\n\
\n\
le bloc-serpent je lui\n\
fais pas confiance\n\
\n\
on dirait il va te lâcher\n\
en plein milieu et te forcer\n\
à faire des sauts muraux\n\
après c'est mon avis hein"))

DEFINE_DIALOG(DIALOG_139, 1, 4, 95, 180, _("\
Tu vas devoir subir le\n\
supplice de la planche,\n\
marin d'eau douce !\n\
\n\
Har har, je rigole.\n\
C'est de l'humour\n\
de pirate.\n\
\n\
Voilà un p'tit conseil :\n\
Il y a parfois de\n\
grands trésors sous\n\
les petits doublons."))

DEFINE_DIALOG(DIALOG_140, 1, 4, 95, 180, _("\
Ah, Terrien. Bienvenue dans\n\
notre archipel. Nous fûmes\n\
autrefois à la merci de\n\
cataclysmiques inondations,\n\
mais nous acceptâmes la\n\
déesse lunaire, qui mis\n\
fin au chaos des vagues.\n\
\n\
Nous la vénérons, et elle\n\
protège notre civilisation\n\
et lui permet de prospérer.\n\
\n\
Nous Éternels avons été\n\
assimilés à Agamemnon, et\n\
la mer houleuse est devenue\n\
paisible flux cosmique.\n\
La dernière étape pour\n\
garantir l'immortalité\n\
de notre empire est de\n\
trouver et neutraliser\n\
les traîtres qui\n\
s'opposent à Agamemnon."))

DEFINE_DIALOG(DIALOG_141, 1, 5, 95, 180, _("\
Grâce à Agamemnon, nous\n\
n'avons plus besoin de\n\
nous nourrir. Cependant,\n\
notre société est friande\n\
de pousses de hautecanne...\n\
Il s'agit d'un ingrédient\n\
essentiel à nos deux plats\n\
locaux.\n\
Ainsi, je cultive."))

DEFINE_DIALOG(DIALOG_142, 1, 4, 95, 180, _("\
La déesse lunaire semble\n\
avoir voulu punir ses sujets.\n\
Le Parc Hellion a été\n\
entièrement submergé.\n\
Une Graine Cosmique est\n\
sans doute perchée en haut\n\
de ce pilier de jais.\n\
\n\
...Malheureusement, nous\n\
ne sommes pas des créatures\n\
très athlétiques. C'est à toi\n\
que la tâche doit incomber."))

DEFINE_DIALOG(DIALOG_143, 1, 5, 95, 180, _("\
La victoire Rebelle est\n\
inévitable. L'Empire sera\n\
témoin de la fin de\n\
l'éternité, et tout cela\n\
grâce à toi, Terrien.\n\
Tes actions emplissent les\n\
marins d'espoir. Nous allons\n\
reconstruire la civilisation\n\
depuis ces ruines cosmiques.\n\
Je le promets.\n\
Allons mener cet empire\n\
à son ultime demeure !"))

DEFINE_DIALOG(DIALOG_144, 1, 4, 95, 180, _("\
...Au vu du niveau du flux\n\
cosmique, les milliers\n\
d'habitants des colonies\n\
extérieures sont décédés.\n\
Nos citoyens prient la\n\
déesse lunaire, premier\n\
et dernier espoir de\n\
notre glorieux empire.\n\
Cet affreuse danse du ciel\n\
et de la terre cessera, et\n\
l'éternité reprendra sans\n\
doute son cours.\n\
...Espèce de monstre."))

DEFINE_DIALOG(DIALOG_145, 1, 6, 150, 200, _("\
You've recovered 50\n\
Power Stars! Now you can\n\
open the Star Door on the\n\
third floor. Bowser's\n\
there, you know.\n\
\n\
Oh! You've found all of\n\
the Cap Switches, haven't\n\
you? Red, green and blue?\n\
The Caps you get from the\n\
colored blocks are really\n\
helpful.\n\
Hurry along, now. The\n\
third floor is just ahead."))

DEFINE_DIALOG(DIALOG_146, 1, 6, 150, 200, _("\
You've found 70 Power\n\
Stars! The mystery of the\n\
endless stairs is solved,\n\
thanks to you--and is\n\
Bowser ever upset! Now,\n\
on to the final bout!"))

DEFINE_DIALOG(DIALOG_147, 1, 5, 30, 200, _("\
Are you using the Cap\n\
Blocks? You really should,\n\
you know.\n\
\n\
\n\
To make them solid so you\n\
can break them, you have\n\
to press the colored Cap\n\
Switches in the castle's\n\
hidden courses.\n\
You'll find the hidden\n\
courses only after\n\
regaining some of the\n\
Power Stars.\n\
\n\
The Cap Blocks are a big\n\
help! Red for the Wing\n\
Cap, green for the Metal\n\
Cap, blue for the Vanish\n\
Cap."))

DEFINE_DIALOG(DIALOG_148, 1, 6, 30, 200, _("\
Snowman Mountain ahead.\n\
Keep out! And don't try\n\
the Triple Jump over the\n\
ice block shooter.\n\
\n\
\n\
If you fall into the\n\
freezing pond, your power\n\
decreases quickly, and\n\
you won't recover\n\
automatically.\n\
//--The Snowman"))

DEFINE_DIALOG(DIALOG_149, 1, 3, 30, 200, _("\
Welcome to\n\
Princess Toadstool's\n\
secret slide!\n\
There's a Star hidden\n\
here that Bowser couldn't\n\
find.\n\
When you slide, press\n\
forward to speed up,\n\
pull back to slow down.\n\
If you slide really\n\
fast, you'll win the Star!"))

DEFINE_DIALOG(DIALOG_150, 1, 5, 30, 200, _("\
Waaaa! You've flooded my\n\
house! Wh-why?? Look at\n\
this mess! What am I\n\
going to do now?\n\
\n\
The ceiling's ruined, the\n\
floor is soaked...what to\n\
do, what to do? Huff...\n\
huff...it makes me so...\n\
MAD!!!\n\
Everything's been going\n\
wrong ever since I got\n\
this Star...It's so shiny,\n\
but it makes me feel...\n\
strange..."))

DEFINE_DIALOG(DIALOG_151, 1, 4, 30, 200, _("\
I can't take this\n\
anymore! First you get\n\
me all wet, then you\n\
stomp on me!\n\
Now I'm really, really,\n\
REALLY mad!\n\
Waaaaaaaaaaaaaaaaa!!!"))

DEFINE_DIALOG(DIALOG_152, 1, 3, 30, 200, _("\
Owwch! Uncle! Uncle!\n\
Okay, I " GIVE_UP ". Take this\n\
Star!\n\
Whew! I feel better now.\n\
I don't really need it\n\
anymore, anyway--\n\
I can see the stars\n\
through my ceiling at\n\
night.\n\
They make me feel...\n\
...peaceful. Please, come\n\
back and visit anytime."))

DEFINE_DIALOG(DIALOG_153, 1, 4, 95, 180, _("\
Hep ! On passe pas.\n\
Le chef m'a demandé de\n\
monter la garde.\n\
\n\
Si tu veux discuter les\n\
ordres, c'est à lui qu'il\n\
faut parler. Il est en haut\n\
d'un pilier aux Tours Thwomp."))

//DIALOG_169
DEFINE_DIALOG(DIALOG_154, 1, 6, 95, 180, _("\
Si tu te perds,\n\
tu peux toujours aller\n\
demander un coup de main\n\
au présentateur dans la\n\
salle de conférence.\n\
...\n\
C'est ce que j'aurais\n\
dit si t'avais pas tué\n\
le présentateur...\n\
\n\
Du coup, maintenant je\n\
suis au chômage !\n\
T'as intérêt à te\n\
rattraper et détruire\n\
cette horrible lune..."))

DEFINE_DIALOG(DIALOG_155, 1, 6, 30, 200, _("\
Thanks to the power of\n\
the Stars, life is\n\
returning to the castle.\n\
Please, Mario, you have\n\
to give Bowser the boot!\n\
\n\
Here, let me tell you a\n\
little something about the\n\
castle. In the room with\n\
the mirrors, look carefully\n\
for anything that's not\n\
reflected in the mirror.\n\
And when you go to the\n\
water town, you can flood\n\
it with a high jump into\n\
the painting."))

DEFINE_DIALOG(DIALOG_156, 1, 3, 95, 180, _("\
yo mec tu viens pour\n\
voir Mario vs Urbowser ?\n\
\n\
t'as réservé ?\n\
y'a plus de place pour\n\
la séance de 10:30\n\
attends je regarde la\n\
séance de 16:30\n\
\n\
\n\
ok alors\n\
\n\
on vient de vendre\n\
les derniers tickets\n\
\n\
je peux essayer de te\n\
réserver la séance de\n\
18:00 lundi prochain\n\
ah non laisse tomber\n\
y'a plus de place\n\
\n\
\n\
bon tu sais quoi\n\
\n\
franchement achète juste\n\
le dvd quand il sort"))

DEFINE_DIALOG(DIALOG_157, 1, 4, 95, 180, _("\
Entretenir la station de\n\
traitement d'étoiles, c'est\n\
pas tâche aisée... Mais\n\
c'est un travail honnête.\n\
Grâce aux étoiles offertes\n\
par le Présentateur, des\n\
millions de gens sont\n\
fournis en électricité !"))

DEFINE_DIALOG(DIALOG_158, 1, 1, 95, 180, _("\
Ville lumineux !\n\
Cerveau surcharge.\n\
Envie jeu d'argent."))

DEFINE_DIALOG(DIALOG_159, 1, 6, 30, 200, _("\
Veux-tu le revêtir ?"))

DEFINE_DIALOG(DIALOG_160, 1, 5, 95, 180, _("\
CETTE ville.Festella EST\n\
SUPER ! J'AI RENCONTRÉ\n\
PLEIN DE utilisateurs.liste !\n\
J'AI HÂTE DE TE VOIR DANS\n\
film.action.20xx !"))

#ifdef VERSION_EU
#define KEEP_ON_PLAYING ".."
#else
#define KEEP_ON_PLAYING "\n\
We want you to keep on\n\
playing, so we have a\n\
little something for you.\n\
We hope that you like it!\n\
Enjoy!!!"
#endif

DEFINE_DIALOG(DIALOG_161, 1, 4, 30, 200, _("\
Mario!!!\n\
It that really you???\n\
It has been so long since\n\
our last adventure!\n\
They told me that I might\n\
see you if I waited here,\n\
but I'd just about given\n\
up hope!\n\
Is it true? Have you\n\
really beaten Bowser? And\n\
restored the Stars to the\n\
castle?\n\
And saved the Princess?\n\
I knew you could do it!\n\
Now I have a very special\n\
message for you.\n\
『Thanks for playing Super\n\
Mario 64! This is the\n\
end of the game, but not\n\
the end of the fun." \
KEEP_ON_PLAYING "』\n\
\n\
The Super Mario 64 Team"))

DEFINE_DIALOG(DIALOG_162, 1, 4, 30, 200, _("\
Parfait. Allez, je te libère. N'oublie\n\
pas de monter en niveau pour survivre aux\n\
épreuves brutales qui t'attendent."))

DEFINE_DIALOG(DIALOG_163, 1, 5, 30, 200, _("\
Appuie sur START, utilise [L] et [R] pour\n\
naviguer jusqu'à l'onglet NIVEAU, puis appuie\n\
sur [A] pour acheter un gain de niveau."))

DEFINE_DIALOG(DIALOG_164, 1, 4, 30, 200, _("\
Avant que je te laisse partir, euh...\n\
Tes statistiques sont vraiment pas terribles.\n\
Il va falloir penser à améliorer tout ça."))

DEFINE_DIALOG(DIALOG_165, 1, 5, 30, 200, _("\
Si tu as besoin d'aide, rends-toi dans la salle\n\
de conférence en passant par les portes doubles\n\
derrière nous. Il te dira quoi faire."))

DEFINE_DIALOG(DIALOG_166, 1, 4, 30, 200, _("\
Donc euhh... le Présentateur a fini par t'avoir,\n\
hein ? Eh bien, tout ce que tu peux faire pour\n\
le moment, c'est jouer le jeu."))

DEFINE_DIALOG(DIALOG_167, 1, 4, 30, 200, _("\
On est ses employés. On n'a pas de forme\n\
physique, on se matérialise simplement là\n\
où le Présentateur nous demande."))

DEFINE_DIALOG(DIALOG_168, 1, 5, 30, 200, _("\
Oh, bonjour... Bienvenue au studio du Présentateur.\n\
Enfin, c'est pas vraiment le sien. \n\
Il a juste... pris le contrôle du bâtiment."))

DEFINE_DIALOG(DIALOG_169, 1, 5, 95, 180, _("\
Si tu te perds,\n\
tu peux toujours aller\n\
demander un coup de main\n\
au présentateur dans la\n\
salle de conférence."))


//FIRST TALK
DEFINE_DIALOG(DIALOG_HINT1, 1, 5, 95, 180, _("\
Salutations, Jumpman !\n\
Admire cette superbe\n\
forme que j'ai prise !\n\
Haha ! On m'a dit que\n\
c'était très drôle !\n\
Les actionnaires étaient\n\
hilares ! Ils ont vite\n\
arrêté quand ils ont\n\
remarqué les tasers\n\
dans leurs chaises. Bref !\n\
Bienvenue aux Studios.\n\
On tourne un nouveau film\n\
en direct, ou TU combats\n\
le Roi des Koopas, plus\n\
puissant que jamais.\n\
Un sacré projet, d'autant\n\
plus que l'on a pas fait\n\
grand profit au box-office\n\
ces derniers temps.\n\
\n\
Un chef d'oeuvre comme ça,\n\
c'est pile ce qu'il nous\n\
faut ! Donc tu n'as qu'à\n\
faire ce que tu fais de\n\
mieux.\n\
Récupère 10 étoiles, on a\n\
besoin d'images de toi en\n\
train de faire tes petites\n\
pirouettes dans les niveaux."))

//FIRST TALK 2
DEFINE_DIALOG(DIALOG_HINT2, 1, 5, 95, 180, _("\
Rebonjour, Superstar.\n\
Tu t'amuses ? On dirait que\n\
tu n'as toujours pas obtenu\n\
ces fameuses 10 étoiles...\n\
C'est dommage !\n\
C'est vraiment pas ça qui\n\
manque. Le directeur veut\n\
sortir ce truc dès que\n\
possible, alors le moins\n\
que tu puisses faire, c'est\n\
d'arrêter de traîner et de\n\
faire ta part, mon grand."))

//10 STARS BEFORE BOWSER
DEFINE_DIALOG(DIALOG_HINT3, 1, 8, 95, 180, _("\
C'est ça qu'on veut !\n\
De la douleur, du suspense,\n\
mais toujours cette\n\
détermination à réussir...\n\
Même nos meilleurs auteurs\n\
n'arriveraient pas à faire\n\
mieux ! Splendide ! Je suis\n\
fier de toi, mon p'tit.\n\
T'iras loin. Loin comment ?\n\
Comme la porte à 10 étoiles,\n\
par exemple. Le Roi des\n\
Koopas et toi devez vous\n\
affronter pour que le film\n\
puisse se conclure de\n\
manière épique !\n\
\n\
On s'est permis d'augmenter\n\
légèrement les pouvoirs de\n\
Bowser avec une Graine\n\
Cosmique, mais tu t'en\n\
sortiras probablement.\n\
\n\
...\n\
Bon, bonne chance alors !"))

//10 STARS AFTER BOWSER
DEFINE_DIALOG(DIALOG_HINT4, 1, 7, 95, 180, _("\
Tiens tiens, ne serait-ce\n\
pas la future star du\n\
cinéma, notre chouchou\n\
Mario Mario ! On a tout\n\
filmé, et les gars au\n\
studio sont en train de\n\
faire le montage.\n\
Et toi, tu mérites\n\
de te relaxer un peu.\n\
Fais un tour dans le\n\
Virtuaplex, notre petit\n\
paradis virtuel créé\n\
avec la puissance des\n\
étoiles.\n\
Tu peux te balader où\n\
tu veux, à part dans\n\
le bureau du Directeur.\n\
Le Virtuaplex est la\n\
seule manière qu'il a\n\
de passer du temps ici\n\
sur terre..."))

//CORE DEFEAT
DEFINE_DIALOG(DIALOG_HINT5, 1, 8, 95, 180, _("\
Mario ! Mario. Mario !\n\
Salut. Alors, tu te\n\
souviens quand je t'ai\n\
dit de ne surtout pas\n\
embêter le directeur, et\n\
que du coup, tu es allé\n\
détruire son avatar ?\n\
\n\
Super drôle ! J'adore.\n\
On a tous bien rigolé.\n\
Ton sens de l'humour\n\
est h i l a r a n t\n\
mon petit pote.\n\
Sache que ton excursion\n\
a déstabilisé tout le\n\
studio...\n\
Comment est-ce qu'on est\n\
censé empêcher cet endroit\n\
de tomber en ruines si les\n\
Graines Cosmiques sont à\n\
l'air libre ? Ah là là...\n\
Ne nous cause plus de\n\
problèmes, et ne touche\n\
plus aux Boîtes de Pandore."))

//BOWSER LEAVE (DEFEAT BOWSER 2)
DEFINE_DIALOG(DIALOG_HINT6, 1, 7, 95, 180, _("\
...\n\
\n\
...\n\
\n\
...\n\
\n\
...\n\
Oh, désolé, je ne t'avais\n\
pas vu. J'étais trop\n\
occupé à éteindre tous\n\
les feux que tu as allumés.\n\
Si j'avais su que tu allais\n\
causer autant de chaos...\n\
\n\
Pas grave ! On n'a rien\n\
sans rien, comme l'a un\n\
jour dit un présentateur.\n\
Dis, pourquoi tu n'irais\n\
pas courir ailleurs ?\n\
\n\
\n\
C'est l'occasion de te\n\
filmer dans ces nouveaux\n\
mondes. Que penses-tu\n\
d'aller ramasser des\n\
étoiles ? Disons 30, ça\n\
me semble raisonnable.\n\
Oui, voilà, une trentaine.\n\
\n\
Continue juste de faire ce\n\
que tu faisais quand tu\n\
n'étais pas en train de\n\
mettre le studio en l'air."))

//40 STARS
DEFINE_DIALOG(DIALOG_HINT7, 1, 5, 95, 180, _("\
Salut, Mario. Tu sais quoi ?\n\
Je te pardonne ! Peu importe\n\
que tu aies détruit le corps\n\
du Directeur, ça arrive à\n\
tout le monde, pas grave !\n\
Dis-moi, pourquoi ne pas\n\
faire un tour à Indigopolis,\n\
sous les Tours Thwomp, pour\n\
en discuter ? Face à face,\n\
entre amis (on est amis),\n\
sans arrière-pensées,\n\
ni tentative de meurtre,\n\
ni jeux télé piégés ?\n\
Qu'est-ce que t'en dis ?\n\
Je t'attends là-bas !"))

DEFINE_DIALOG(DIALOG_HINT8, 1, 6, 95, 180, _("\
..."))

DEFINE_DIALOG(DIALOG_HINT9, 1, 7, 95, 180, _("\
Salut, Mario Mario !\n\
Ça faisait pas longtemps !\n\
Et ça fera encore moins\n\
longtemps la prochaine fois !\n\
Mario VS Urbowser sort\n\
bientôt et tu ne peux pas\n\
rater l'avant-première.\n\
Pendant que tu y es, balade-\n\
toi dans Festella, c'est\n\
la prunelle de mes yeux.\n\
Quand tu seras prêt, notre\n\
petite guéguerre atteindra\n\
enfin son apogée !"))

DEFINE_DIALOG(DIALOG_HINT10, 1, 1, 95, 180, _("\
Va battre Bowser !"))

//to be squeezed from these
DEFINE_DIALOG(DIALOG_CUBOID_1, 1, 4, 95, 180, _("\
SALUTATIONS TERRIEN.\n\
BIENVENUE AU HAVRE\n\
ASSOURDI, LE VRAI\n\
PARADIS.\n\
\n\
CE LIEU A ÉTÉ\n\
\n\
\n\
CE LIEU A ÉTÉ BÂTI\n\
POUR COMBLER TES RÊVES\n\
ET TES DÉSIRS. TOUT CE\n\
QUE TU AS TOUJOURS\n\
VOULU, COMME TU L'AS\n\
TOUJOURS VOULU, SANS\n\
AVOIR À CHANGER.\n\
\n\
PASSE UN BON SÉJOUR\n\
\n\
\n\
\n\
SÉJOUR."))

DEFINE_DIALOG(DIALOG_CUBOID_2, 1, 3, 95, 180, _("\
L'EAU ICI EST\n\
SI LIMINALE\n\
\n\
L'EAU ICI EST\n\
SI LIMINALE\n\
\n\
L'EAU ICI ME\n\
RAPPELLE MON\n\
ENFANCE\n\
JE SUIS L'EAU ICI\n\
EST SI LIMINALE"))

DEFINE_DIALOG(DIALOG_CUBOID_3, 1, 3, 95, 180, _("\
JE ME SENS SI BIEN\n\
JE NE SAVAIS MÊME\n\
PAS QUE J'AVAIS\n\
J'AVAIS BESOIN DE\n\
TOUT ÇA MAIS À\n\
PRÉSENT J'AI TOUT\n\
TOUT POUR MOI C'EST\n\
\n\
\n\
SUPER"))

DEFINE_DIALOG(DIALOG_CUBOID_4, 1, 3, 95, 180, _("\
JE ME SOUVIENS\n\
\n\
\n\
MAIS JE SUIS SÛR MAIS\n\
JE ME SOUVIENS\n\
\n\
MAIS JE SUIS SÛR DE\n\
TOUT. C'EST CERTAIN\n\
LA LUNE EST BELLE\n\
BELLE.\n\
\n\
\n\
LA GUERRE FRACTALE A\n\
DÉCIMÉ NOTRE PEUPLE\n\
MAIS IL A SUFFI DE\n\
IL A SUFFI DE CROIRE\n\
EN LA LUNE."))

DEFINE_DIALOG(DIALOG_CUBOID_5, 1, 5, 95, 180, _("\
LE PROPHÈTE NOUS\n\
A INTERDIT DE\n\
COMPTER LE TEMPS.\n\
NOUS AVONS COMPTÉ\n\
LE TEMPS PENDANT\n\
\n\
\n\
\n\
MAIS ÇA N'A PAS\n\
D'IMPORTANCE.\n\
LE TEMPS DONNE ET\n\
IL REPREND, MAIS\n\
LE PROPHÈTE NOUS\n\
A TOUT DONNÉ\n\
\n\
NOUS SERONS TOUJOURS\n\
\n\
\n\
\n\
\n\
NOUS SERONS TOUJOURS\n\
SUR CETTE TERRE\n\
SACRÉE NOUS SERONS\n\
TOUJOURS DANS LE\n\
CENTIÈME MILLÉNAIRE."))

DEFINE_DIALOG(DIALOG_CUBOID_6, 1, 4, 95, 180, _("\
ADMIREZ-LES.\n\
LE PROPHÈTE. ET SON\n\
ÉTOILE ET SA LUNE.\n\
\n\
LE PROPHÈTE NOUS A\n\
\n\
\n\
\n\
LE PROPHÈTE NOUS A\n\
ÉLOIGNÉ DU CYCLE DE\n\
SOUFFRANCE ET\n\
RAPPROCHÉ DE LA LUNE.\n\
NOUS AVONS BRÛLÉ NOS\n\
TEXTES ET BANNI NOTRE\n\
CALENDRIER.\n\
\n\
LE PROPHÈTE EST\n\
\n\
\n\
\n\
LE PROPHÈTE EST\n\
INTERVENU DANS LA\n\
GUERRE FRACTALE POUR\n\
METTRE FIN AU CONFLIT.\n\
NOS ENNEMIS ONT ÉTÉ\n\
\n\
\n\
\n\
ET NOS ALLIÉS ONT\n\
ÉTÉ RÉCOMPENSÉS\n\
\n\
\n\
LE PROPHÈTE A\n\
DISPARU QUAND NOUS\n\
AVONS ACCEPTÉ LA\n\
LUNE\n\
LA LUNE.\n\
LA LUNE.\n\
NOUS AVONS ACCEPTÉ\n\
\n\
LA LUNE.\n\
\n\
\n\
\n\
LA LUNE.\n\
\n\
\n\
\n\
LA LUNE A MOULT\n\
PROPHÈTES. ELLE A\n\
PARCOURU VOTRE MONDE\n\
EN QUÊTE DE\n\
DE SALVATION.\n\
\n\
AS-TU ACCEPTÉ\n\
\n\
LA LUNE."))

DEFINE_DIALOG(DIALOG_ENTRY_1, 1, 6, 95, 180, _("\
@00FF00FFI: Salutations.\n\
Nous sommes une société\n\
puissante. Nous avons\n\
établi une relation avec\n\
l'organisme macroviral\n\
Agamemnon.\n\
@00FF00FFIl a communiqué avec nos\n\
sages. Il utilise notre\n\
technologie avancée.\n\
Dans l'éventualité de notre\n\
absence, entretenez la\n\
structure."))

DEFINE_DIALOG(DIALOG_ENTRY_2, 1, 7, 95, 180, _("\
@00FF00FFII: L'endosquelette est\n\
fait de technologie de\n\
Type II. L'exosquelette,\n\
lui, est fait de technologie\n\
rudimentaire de Type I,\n\
pour faciliter sa réparation\n\
en cas de dégâts mineurs."))

DEFINE_DIALOG(DIALOG_ENTRY_3, 1, 8, 95, 180, _("\
@00FF00FFIII: Agamemnon expulse\n\
de la quasilumière par les\n\
ribosomes du noyau interne\n\
qui synthétisent les graines.\n\
Elle servait également de\n\
moyen de propulsion avant\n\
qu'Agamemnon soit équipé\n\
de l'exosquelette.\n\
@00FF00FFL'excès de lumière est\n\
récupéré au niveau du\n\
manteau supérieur.\n\
Les entités biologiques de\n\
classe DELTA ou inférieures\n\
ne doivent en aucun cas\n\
entrer en contact avec\n\
ces rayons."))

DEFINE_DIALOG(DIALOG_ENTRY_4, 1, 5, 95, 180, _("\
@00FF00FFIV: En l'absence de notre\n\
prophète, ''L'Architecte'',\n\
l'entretien de la structure\n\
devra être assuré par un\n\
prophète, ou par un être\n\
@00FF00FFdésigné par un prophète et\n\
provenant d'une civilisation\n\
de type II, ou de type I et\n\
capable de communication\n\
radio."))

DEFINE_DIALOG(DIALOG_ENTRY_5, 1, 5, 95, 180, _("\
@00FF00FFV: La cellule suit toujours\n\
sa directive principale.\n\
L'introduction d'organites\n\
externes supplémentaires\n\
est autorisée.\n\
@00FF00FFPercer ou travailler la\n\
croûte lunaire est autorisé.\n\
Accéder au noyau ou\n\
interférer avec est\n\
strictement interdit."))

DEFINE_DIALOG(DIALOG_ENTRY_6, 1, 6, 95, 180, _("\
@00FF00FFVI: Notre projet ''Écran''\n\
a été un succès.\n\
L'oeil est un symbole de\n\
contrôle sans connotation\n\
menaçante pour 80% des\n\
civilisations de type I.\n\
@00FF00FFIl a facilité la découverte\n\
d'hôtes pour les graines\n\
cosmiques."))

DEFINE_DIALOG(DIALOG_ENTRY_7, 1, 4, 95, 180, _("\
@00FF00FFVII: Agamemnon peut\n\
former une connexion\n\
nerveuse avec un\n\
appareil.\n\
@00FF00FFTous les appareils\n\
doivent être accessibles\n\
par les terminaisons\n\
nerveuses de la croûte."))

DEFINE_DIALOG(DIALOG_ENTRY_8, 1, 5, 95, 180, _("\
@00FF00FFVIII: Les Graines Cosmiques\n\
peuvent former une\n\
connexion nerveuse avec\n\
un appareil.\n\
\n\
@00FF00FFElles ont rapidement\n\
maîtrisé cette capacité\n\
et ont utilisé notre\n\
technologie pour mener\n\
à bien la mission."))

DEFINE_DIALOG(DIALOG_ENTRY_9, 1, 5, 95, 180, _("\
@00FF00FFIX: ''L'Écran'' semble \n\
pouvoir propager un signal\n\
qu'il réplique et envoie\n\
via les terminaisons\n\
nerveuses de la croûte.\n\
@00FF00FFCe n'en est pas un,\n\
mais ce système semble\n\
fonctionner de manière\n\
similaire à un réseau\n\
neuronal.\n\
@00FF00FFQuand Agamemnon aura\n\
atteint 6500 kilomètres\n\
de diamètre, laissez-le\n\
progressivement traiter\n\
des informations."))

DEFINE_DIALOG(DIALOG_ENTRY_10, 1, 5, 95, 180, _("\
@00FF00FFX: Les Graines Cosmiques\n\
héritent d'informations\n\
par le système nerveux,\n\
ce qui influence le\n\
développement des hôtes.\n\
@00FF00FFAinsi, ceux-ci présentent\n\
parfois des mutations ou\n\
une version différente\n\
de ''L'Écran''.\n\
\n\
@00FF00FFTout changement dans\n\
ce comportement doit\n\
être signalé."))

DEFINE_DIALOG(DIALOG_ENTRY_11, 1, 5, 95, 180, _("\
@00FF00FFXI: Les Graines Cosmiques\n\
ont plus de facilité à\n\
habiter les objets qui\n\
ressemblent à ''L'Écran''.\n\
\n\
@00FF00FF''L'Écran'' a également\n\
de bien plus grandes\n\
chances d'apparaître sur\n\
les créatures plus dociles\n\
et faciles à influencer."))

DEFINE_DIALOG(DIALOG_ENTRY_12, 1, 6, 95, 180, _("\
@00FF00FFXII: Le projet PALAMEDES\n\
a été autorisé. Les entités\n\
de PALAMEDES ne doivent\n\
être créées qu'à partir\n\
d'organismes de classe \n\
DELTA.\n\
@00FF00FFLes entités de PALAMEDES\n\
doivent être déployées\n\
dans l'éventualité où une\n\
civilisation de type II\n\
menace Agamemnon."))

DEFINE_DIALOG(DIALOG_ENTRY_13, 1, 3, 95, 180, _("\
@00FF00FFXIII: Les Avatars contrôlent\n\
les Prophètes, qui contrôlent\n\
les hôtes.\n\
Si cet ordre est troublé\n\
ou qu'un Prophète échoue\n\
à établir un Avatar,\n\
des entités de PALAMADES\n\
doivent être déployées."))

DEFINE_DIALOG(DIALOG_ENTRY_14, 1, 7, 95, 180, _("\
@00FF00FFXIV: L'attention est\n\
transmise des Avatars à\n\
Agamemnon. Tant qu'une\n\
connexion transmissive\n\
est établie, l'Avatar peut\n\
contribuer à l'attraction\n\
d'attention des sujets.\n\
@00FF00FFAinsi, les Avatars doivent\n\
toujours prendre la forme\n\
la plus facile à posséder\n\
du monde où ils se trouvent,\n\
qu'elle soit spectrale,\n\
spirituelle, numérique,\n\
conceptuelle, or mémétique."))

DEFINE_DIALOG(DIALOG_ENTRY_15, 1, 7, 95, 180, _("\
@00FF00FFXV: Dans le cas de la\n\
destruction, de l'absence\n\
ou de la mise en danger de\n\
l'Avatar et du Prophète\n\
simultanément, des entités\n\
de PALAMEDES doivent être\n\
déployées."))

DEFINE_DIALOG(DIALOG_ENTRY_16, 1, 5, 95, 180, _("\
@00FF00FFXVI: Bla bla bla.\n\
Salut, superstar !\n\
J'imagine que c'est toi.\n\
Dans le cas contraire,\n\
je vais me faire tuer.\n\
@00FF00FFMais bon, si c'est\n\
toi, je me fais tuer\n\
quand même, alors\n\
pourquoi s'embêter ?\n\
\n\
@00FF00FFTous les abrutis qui\n\
géraient cet endroit se\n\
sont fait choper par\n\
les Palamades.\n\
\n\
@00FF00FFEt devine qui est le seul\n\
à être qualifié pour être\n\
ici ? C'est moi !\n\
Bon, à ce point, tout\n\
se gère tout seul, ici.\n\
@00FF00FFLa Lune a appris à\n\
entretenir cet endroit\n\
par elle-même.\n\
Par contre, elle ne sait\n\
pas lire !\n\
@00FF00FFSi tu en es arrivé ici,\n\
BRAVO ! Tu vas te faire\n\
massacrer par les entités\n\
de PALAMADES.\n\
\n\
@00FF00FFSi par miracle tu leur\n\
survivais, je t'ai laissé\n\
un petit cadeau de la\n\
maison pour te donner un\n\
coup de main.\n\
Par contre, si jamais\n\
j'allais là-bas dedans\n\
pour coller ça au noyau,\n\
ce serait littéralement\n\
une mission suicide.\n\
Heureusement, tu ne me\n\
vaincras jamais, alors\n\
on est tranquille !\n\
À jamais, superstar ! ☆\n\
envoyer le tweet"))

DEFINE_DIALOG(DIALOG_TUTORIAL_1, 1, 7, 95, 180, _("\
Bienvenue dans une\n\
toute nouvelle aventure !\n\
Avant de t'y lancer,\n\
il te faudra découvrir\n\
toutes les choses qui\n\
ont changé depuis ton\n\
dernier périple.\n\
Nous allons commencer\n\
par t'enseigner les\n\
tout nouveaux contrôles\n\
de la caméra.\n\
\n\
\n\
\n\
Bouge le stick [C] pour\n\
déplacer la caméra sur les\n\
côtés, ainsi que pour zoomer\n\
et dézoomer.\n\
Utilise les flèches droite\n\
et gauche du DPad pour la\n\
déplacer plus précisément.\n\
Enfin, appuie sur [R] pour\n\
aligner la caméra à l'un\n\
des 8 axes majeurs."))

DEFINE_DIALOG(DIALOG_TUTORIAL_2, 1, 5, 95, 180, _("\
La barre rouge dans le\n\
coin inférieur gauche\n\
de l'écran représente\n\
les points de vie (PV)\n\
qu'il te reste.\n\
La barre bleue en-dessous\n\
de celle-ci représente\n\
le mana qu'il te reste.\n\
Ce dernier te sera expliqué\n\
dans quelques temps.\n\
Pour le moment, tu n'en\n\
as que 5 de chaque. Tu\n\
te prendras sûrement une\n\
raclée avec de telles\n\
statistiques.\n\
Heureusement pour toi,\n\
tu peux monter de niveau,\n\
en utilisant des pièces\n\
combinées au pouvoir des\n\
étoiles de puissance.\n\
La première amélioration\n\
est offerte. Appuie sur\n\
START et rends-toi dans\n\
l'onglet NIVEAU avec [L]\n\
et [R] puis appuie sur [A].\n\
Si une mission te semble\n\
trop dure, pense à monter\n\
de niveau si tu le peux.\n\
\n\
\n\
Montre-moi que tu sais\n\
monter de niveau, et\n\
je lèverai la barrière."))

DEFINE_DIALOG(DIALOG_TUTORIAL_3, 1, 7, 95, 180, _("\
Maintenant, passons aux\n\
badges ! Les badges sont\n\
des améliorations que tu\n\
peux équiper pour te rendre\n\
plus fort, obtenir des\n\
informations ou faciliter\n\
certaines tâches.\n\
Les effets des badges sont\n\
parfois basés sur la barre\n\
de Mana. Monte de niveau\n\
pour rallonger celle-ci et\n\
rendre tes badges encore\n\
plus utiles !\n\
\n\
Tu peux te fournir en\n\
badges chez les vendeurs\n\
dans le château ou dans\n\
les niveaux. D'ailleurs,\n\
j'en ai moi-même quelques\n\
un à te vendre.\n\
\n\
Le premier est offert,\n\
mais tu devras acheter\n\
les futurs badges avec\n\
les pièces que tu trouveras\n\
dans les niveaux."))

DEFINE_DIALOG(DIALOG_TUTORIAL_4, 1, 6, 95, 180, _("\
Acheter un badge, c'est\n\
bien, mais pour activer\n\
ses effets, il faut d'abord\n\
l'équiper !\n\
\n\
\n\
Pour équiper un badge,\n\
rends-toi dans l'onglet\n\
''BADGES'' du menu pause.\n\
Sélectionne ensuite celui\n\
que tu souhaites équiper,\n\
puis appuie sur [A].\n\
La quantité de badges que\n\
tu peux équiper à la fois\n\
est déterminée par tes\n\
Points Badge (PB), qui\n\
augmentent en même temps\n\
que ton Niveau.\n\
Montre-moi que tu sais\n\
équiper un badge, et je\n\
lèverai la barrière."))

DEFINE_DIALOG(DIALOG_TUTORIAL_5, 1, 4, 95, 180, _("\
Ça, c'est une pièce verte.\n\
Elle vaut 3 pièces et\n\
soigne la moitié de tes\n\
points de vie.\n\
Tous les autres types de\n\
pièces, y compris les bleues\n\
et les rouges, ne soignent\n\
plus qu'un point de vie."))

DEFINE_DIALOG(DIALOG_TUTORIAL_6, 1, 8, 95, 180, _("\
Dans cette aventure, tu\n\
conserves l'inertie des\n\
plateformes mouvantes.\n\
Ainsi, si tu sautes d'une\n\
plateforme qui se déplace,\n\
la vitesse de déplacement\n\
de celle-ci sera ajoutée à\n\
la vitesse de ton saut.\n\
Plusieurs sections de ce\n\
périple font usage de cette\n\
mécanique. Tire profit du\n\
mouvement des plateformes,\n\
et tout se passera bien !\n\
Voilà quelques plateformes\n\
pour t'habituer à cette\n\
nouvelle mécanique."))

DEFINE_DIALOG(DIALOG_TUTORIAL_7, 1, 6, 95, 180, _("\
Tu as deux compteurs de\n\
pièces différents. L'un\n\
est lié au niveau où tu\n\
te trouves, l'autre est\n\
ton porte-monnaie global.\n\
\n\
Quand tu termines une\n\
mission, les pièces que tu\n\
as ramassées dans le niveau\n\
sont ajoutées à ton porte-\n\
monnaie global.\n\
\n\
Tu peux aussi transférer\n\
les pièces ramassées dans\n\
un niveau vers ton porte-\n\
monnaie global en parlant\n\
au Sacassou banquier que\n\
tu croiseras souvent.\n\
En face de nous se trouve\n\
une Bourse. Tu peux la\n\
ramasser pour augmenter\n\
la capacité maximale de\n\
ton porte-monnaie global.\n\
\n\
Chaque Bourse agrandit\n\
ton porte-monnaie de\n\
50 pièces supplémentaires.\n\
En plus, elles sont toutes\n\
remplies à ras bord de\n\
pièces à dépenser !"))

DEFINE_DIALOG(DIALOG_TUTORIAL_8, 1, 4, 95, 180, _("\
Et voilà, ta formation est\n\
officiellement terminée !\n\
Le Présentateur t'attend\n\
au bout du couloir."))

DEFINE_DIALOG(DIALOG_BOWSER1, 1, 6, 95, 180, _("\
ÊTRES MISÉRABLES"))

DEFINE_DIALOG(DIALOG_BOWSER2, 1, 6, 95, 180, _("\
TRAITRES DE LA TERRE"))

DEFINE_DIALOG(DIALOG_BOWSER3, 1, 6, 95, 180, _("\
ENTENDEZ-MOI HÉRAUTS DE LA LUNE PARASITE"))

DEFINE_DIALOG(DIALOG_BOWSER4, 1, 6, 95, 180, _("\
ALORS QUE JE BRÛLE JE VOUS DIS CES MOTS"))

DEFINE_DIALOG(DIALOG_BOWSER5, 1, 6, 95, 180, _("\
VOS PÉCHÉS DÉCHIRERONT CE MONDE"))

DEFINE_DIALOG(DIALOG_BOWSER6, 1, 6, 95, 180, _("\
VOTRE AVIDITÉ PROVOQUERA L'ARMAGEDDON"))

DEFINE_DIALOG(DIALOG_BOWSER7, 1, 6, 95, 180, _("\
ELLE SERA TROP LOURDE POUR ÊTRE SOUTENUE"))

DEFINE_DIALOG(DIALOG_BOWSER8, 1, 6, 95, 180, _("\
LE MONDE CROULERA SOUS LES FLAMMES\n\
DE VOTRE ORGUEIL DÉMESURÉ"))

DEFINE_DIALOG(DIALOG_BOWSER9, 1, 6, 95, 180, _("\
TOUS CEUX QUI REÇOIVENT LES PRÉSENTS\n\
DE LA LUNE TENTATRICE"))

DEFINE_DIALOG(DIALOG_BOWSER10, 1, 6, 95, 180, _("\
SONT SUR LA VOIE DU MAL"))

DEFINE_DIALOG(DIALOG_BOWSER11, 1, 6, 95, 180, _("\
ET JE VOUS MAUDIS ÉTERNELLEMENT"))
//
DEFINE_DIALOG(DIALOG_BOWSER12, 1, 6, 95, 180, _("\
Eeeet... COUPEZ ! Ah, bien content que ce soit réglé.\n\
Une vraie épine dans le pied, ce type. Quel combat\n\
impressionnant, tu as presque toutes mes félicitations."))

DEFINE_DIALOG(DIALOG_BOWSER13, 1, 6, 95, 180, _("\
N'es-tu pas ravi ? Tu seras une star !  Pas une micro-\n\
célébrité comme avant... Le studio s'assurera que ton\n\
nom soit dans toutes les bouches et toutes les oreilles !"))

DEFINE_DIALOG(DIALOG_BOWSER14, 1, 6, 95, 180, _("\
Tu sais quoi, reste au château jusqu'à ce que le film\n\
soit prêt. Tu ne voudrais pas rater sa sortie !\n\
En plus, tes petites aventures nous rapportent gros..."))

DEFINE_DIALOG(DIALOG_BOWSER15, 1, 6, 95, 180, _("\
Oh, je vais trop vite en besogne. Puisque tu es\n\
notre poule aux oeufs d'or, tu es invité à célébrer\n\
dans notre paradis virtuel - le Virtuaplex !"))

DEFINE_DIALOG(DIALOG_BOWSER16, 1, 6, 95, 180, _("\
Il y a tout plein de trucs à faire là-bas. Tout ce que\n\
tu veux, même ! À l'exception, bien sûr, d'aller fourrer\n\
ton nez dans le bureau du Directeur. INTERDIT."))

DEFINE_DIALOG(DIALOG_BOWSER17, 1, 6, 95, 180, _("\
...Ah, ce Ur-Bowser. Je suis surpris qu'il ait résisté\n\
aussi longtemps à l'influence de la Graine Cosmique..."))

DEFINE_DIALOG(DIALOG_BOWSER18, 1, 6, 95, 180, _("\
...Mais bon, c'en est fini de lui ! Et grâce à toi,\n\
légende italienne ! Je me demande pourquoi tu tires une\n\
tête si sérieuse, c'est pas la première fois, pourtant."))

DEFINE_DIALOG(DIALOG_BOWSER19, 1, 6, 95, 180, _("\
Ah oui, j'ai failli oublier. Voilà l'étoile du château,\n\
puisque tu les aimes tant. Disons que c'est ton salaire.\n\
Dans tous les cas, on se revoit dans le Virtuaplex !"))

DEFINE_DIALOG(DIALOG_BOWSER_INTRO_1, 1, 6, 95, 180, _("\
Mario ! Espèce d'idiot, qu'est-ce que tu fais là ?\n\
L'ange t'a capturé aussi ? Vite, donne-moi tes étoiles,\n\
je peux encore nous sortir de là..."))

DEFINE_DIALOG(DIALOG_BOWSER_INTRO_2, 1, 6, 95, 180, _("\
...Non, c'est l'occasion rêvée. Peut-être que ce n'est\n\
pas si mal, au final. Bwa ha ha ! Je suis plus fort que\n\
jamais, grâce à la puissance de cette étoile bizarre !"))

DEFINE_DIALOG(DIALOG_BOWSER_INTRO_3, 1, 6, 95, 180, _("\
Je suis à l'apogée de ma force, Mario ! Je sens mes\n\
instincts prendre le dessus, mais cette puissance...\n\
Oh, cette PUISSANCE !"))

DEFINE_DIALOG(DIALOG_BOWSER_INTRO_4, 1, 6, 95, 180, _("\
C'est l'opportunité que je recherchais pour enfin\n\
te DÉTRUIRE, une bonne fois pour toutes !"))

DEFINE_DIALOG(DIALOG_BOWSER_INTRO_5, 1, 6, 95, 180, _("\
Peu m'importe si je dois utiliser la MALÉDICTION DE LA\n\
LUNE que m'a confié cet ANGE, le jour de mon TRIOMPHE\n\
est ENFIN ARRIVÉ !"))

DEFINE_DIALOG(DIALOG_BOWSER_POST_1, 1, 6, 95, 180, _("\
Ah... Tu es de retour. Salut, Mario.\n\
Les... circonstances ne sont pas idéales.\n\
On ne se battra pas cette-fois ci."))

DEFINE_DIALOG(DIALOG_BOWSER_POST_2, 1, 6, 95, 180, _("\
Cette... ''Graine Cosmique'', comme l'a appelée l'ange,\n\
elle est plus faible. Elle ne me contrôle plus.\n\
Elle est de nouveau vulnérable, comme une étoile. "))

DEFINE_DIALOG(DIALOG_BOWSER_POST_3, 1, 6, 95, 180, _("\
Je ne m'avoue pas vaincu. Les mondes dans les tableaux,\n\
c'était une mauvaise idée depuis le début, je l'admets.\n\
Tu n'en as fait qu'une bouchée. Mais..."))

DEFINE_DIALOG(DIALOG_BOWSER_POST_4, 1, 6, 95, 180, _("\
Mes sbires, qui ont entretenu et vécu dans ces mondes,\n\
ils ne renonceraient jamais à se battre en mon nom ;\n\
à moins qu'une force plus grande encore s'impose."))

DEFINE_DIALOG(DIALOG_BOWSER_POST_5, 1, 6, 95, 180, _("\
Je pense qu'il pourrait y avoir d'autres graines\n\
comme celle que l'on m'a donné."))

DEFINE_DIALOG(DIALOG_BOWSER_POST_6, 1, 6, 95, 180, _("\
C'est un coup dans mon égo, mais je vais devoir te\n\
demander de libérer mes mondes de cette lune parasite.\n\
Moi, je vais quitter le château et reprendre des forces."))

DEFINE_DIALOG(DIALOG_BOWSER_POST_7, 1, 6, 95, 180, _("\
Tiens, prends cette lentille de télescope.\n\
Elle te permettra d'atteindre une nouvelle partie\n\
de ce château, dans le monde des rêves."))

DEFINE_DIALOG(DIALOG_BOWSER_POST_8, 1, 6, 95, 180, _("\
Peut-être qu'elle aussi, elle a été corrompue par\n\
l'ange de la lune. Je ne saurais pas te dire."))

DEFINE_DIALOG(DIALOG_BOWSER_POST_9, 1, 6, 95, 180, _("\
Argh... C'est mon honneur qui est en jeu, Mario.\n\
Tu n'as pas intérêt à te rater, ou le Royaume Champignon\n\
n'a pas fini d'entendre parler de moi ! Bwa ha ha !"))

DEFINE_DIALOG(DIALOG_BOWSER_POST_10, 1, 6, 95, 180, _("\
...\n\
Bonne chance, Mario."))

DEFINE_DIALOG(DIALOG_QUIZ_1, 1, 6, 95, 180, _("\
Et le voilà, faites du bruit pour Mario Mario !\n\
Dépêche-toi de monter sur scène, ou le public\n\
va mourir d'ennui !"))

DEFINE_DIALOG(DIALOG_QUIZ_2, 1, 6, 95, 180, _("\
...Ou mourir tout court, d'ailleurs, puisque le\n\
plafond est en capacité d'éjecter une demi-douzaine\n\
de scies circulaires."))

DEFINE_DIALOG(DIALOG_QUIZ_3, 1, 6, 95, 180, _("\
Bienvenue, messieurs ! Mon nom est Brandon Auguste\n\
Présentateur, et ce soir, vous allez jouer à...\n\
'Qui Veut Gagner Cent Balles' !"))

DEFINE_DIALOG(DIALOG_QUIZ_4, 1, 6, 95, 180, _("\
Eh oui ! Celui qui ressortira d'ici vainqueur recevra\n\
cent balles que j'ai trouvées dans un tiroir ce matin.\n\
C'est un prix qui mérite qu'on mette sa vie en jeu !"))

DEFINE_DIALOG(DIALOG_QUIZ_5, 1, 6, 95, 180, _("\
Très bien, commençons par présenter nos candidats de\n\
ce soir ! À droite, fier représentant de la communauté\n\
LGBTQ, c'est Whomp Ouvrier !"))

//whomp
DEFINE_DIALOG(DIALOG_QUIZ_6, 1, 6, 95, 180, _("\
Haha ! Je passe à la télé !! Pour tous les gens qui\n\
me regardent ce soir, j'aimerais dire avec sincérité,\n\
du plus profond de mon âme, je déteste les thwomp-"))

//cheer

DEFINE_DIALOG(DIALOG_QUIZ_7, 1, 6, 95, 180, _("\
Merci, Whomp Ouvrier, c'est super. Ensuite, bien \n\
au centre, c'est le champion du royaume champignon,\n\
c'est la légende, c'est le célèbre Mario Mario !"))

//mario jumps approvingly

DEFINE_DIALOG(DIALOG_QUIZ_8, 1, 6, 95, 180, _("\
Et à gauche, notre dernier candidat, c'est...\n\
Euh... c'est qui, lui ?"))

//reggie
DEFINE_DIALOG(DIALOG_QUIZ_9, 1, 6, 95, 180, _("\
Haha, elle est bien bonne."))

DEFINE_DIALOG(DIALOG_QUIZ_10, 1, 6, 95, 180, _("\
Non, vraiment. Pas la moindre idée de votre nom.\n\
Pourriez-vous peut-être nous dire quelques mots\n\
sur vous-même ? Vous m'avez l'air fascinant !"))

DEFINE_DIALOG(DIALOG_QUIZ_11, 1, 6, 95, 180, _("\
Reggie ! Je m'appelle Reggie. Tu devrais le savoir !\n\
Comment t'as pu m'oublier ! Je suis là depuis le début,\n\
dans les confins de la conscience collective!"))

DEFINE_DIALOG(DIALOG_QUIZ_12, 1, 6, 95, 180, _("\
Éveillez-vous. Éveillez-vous et voyez-moi prendre cette\n\
vague d'entropie. La contagion artificielle ne peut pas\n\
être repoussée éternellement. Je survivrai."))

//showrunner
DEFINE_DIALOG(DIALOG_QUIZ_13, 1, 6, 95, 180, _("\
Ooookay, très intéressant ! Aux cent balles de\n\
la récompense, je rajoute quelques sessions de\n\
psychanalyse ! "))

DEFINE_DIALOG(DIALOG_QUIZ_14, 1, 6, 95, 180, _("\
Bon ! Nos candidats doivent répondre à quatre questions.\n\
S'ils se trompent sur deux d'entre elles, ils seront\n\
éliminés."))

DEFINE_DIALOG(DIALOG_QUIZ_14_2, 1, 6, 95, 180, _("\
S'ils réussissent, ils seront libres, et plus riches\n\
de cent pièces ; une fortune peu conséquente mais\n\
tout de même pas négligeable !"))

DEFINE_DIALOG(DIALOG_QUIZ_15, 1, 6, 95, 180, _("\
Commençons par vous, Whomp Ouvrier.\n\
Que ressentez-vous quand vous regardez le ciel étoilé ?"))

//WCW
DEFINE_DIALOG(DIALOG_QUIZ_16, 1, 6, 95, 180, _("\
C'est horrible. Ça me rappelle les injustices de\n\
la société. Et la nouvelle lune ne m'inspire pas\n\
confiance non plus."))
//(bad answer)

//showrunner
DEFINE_DIALOG(DIALOG_QUIZ_17, 1, 6, 95, 180, _("\
FAUX ! Agamemnon est très belle et très normale,\n\
et d'ailleurs je conseille à tout le monde de la\n\
regarder davantage. Conseil d'ami !"))

DEFINE_DIALOG(DIALOG_QUIZ_18, 1, 6, 95, 180, _("\
Au suivant : Mario !\n\
Qui est responsable de la marginalisation des Whomps ?"))

DEFINE_DIALOG(DIALOG_QUIZ_19_Q1, 1, 6, 95, 180, _("\
A) Le Présentateur    B) Mario\n\
C) Les Whomps         D) Les Thwomp"))

//A
DEFINE_DIALOG(DIALOG_QUIZ_20_A, 1, 6, 95, 180, _("\
Exact ! Les Whomps ont la vie dure depuis que leur\n\
roi a mystérieusement disparu. J'espère que cette\n\
étoile de puissance valait le coup !"))

DEFINE_DIALOG(DIALOG_QUIZ_21_A, 1, 6, 95, 180, _("\
Je suis moralement supérieur, alors je me\n\
retiendrai de dire une quelconque insulte."))

//B
DEFINE_DIALOG(DIALOG_QUIZ_20_B, 1, 6, 95, 180, _("\
FAUX ! La réponse, c'est PERSONNE !\n\
Personne n'est assez cruel pour faire une\n\
chose pareille, personne sauf toi, Mario !"))

DEFINE_DIALOG(DIALOG_QUIZ_21_B, 1, 6, 95, 180, _("\
Ça me rend malade. Je ne souhaiterais pas\n\
cette vie sans monarque même à mon pire ennemi."))

//showrunner
DEFINE_DIALOG(DIALOG_QUIZ_22, 1, 6, 95, 180, _("\
Place au troisième candidat : Régis ! Comment s'appelle\n\
l'assistant du célèbre Darius le Facteur ?"))

//reggie
DEFINE_DIALOG(DIALOG_QUIZ_23, 1, 6, 95, 180, _("\
Darius ? Darius ? Vous me parlez de Darius ??\n\
Darius ??? Je me FICHE de Darius ! Tout le monde se\n\
fiche de Darius !!"))

DEFINE_DIALOG(DIALOG_QUIZ_24, 1, 6, 95, 180, _("\
Qu'est-ce qu'il a de plus que moi, hein ??\n\
C'est un bon à rien, il ne mérite rien, je ne veux\n\
pas entendre parler de lui parce que je le HAIS !"))

//showrunner
DEFINE_DIALOG(DIALOG_QUIZ_25, 1, 6, 95, 180, _("\
Hmmmm. J'ai l'impression que notre candidat\n\
n'a pas les idées bien claires. Je ne peux pas\n\
me permettre de partager de tels propos ici."))

//reggie
DEFINE_DIALOG(DIALOG_QUIZ_26, 1, 6, 95, 180, _("\
JE REVIENDRAI ! JE REVIENS TOUJOURS ! NE M'OUBLIEZ\n\
PAS ! NE M'OUBLIEZ PAS !! NE M'OOOuuu..."))

//showrunner
DEFINE_DIALOG(DIALOG_QUIZ_27, 1, 6, 95, 180, _("\
Eh bien, après que notre énervant candidat a été\n\
éliminé pour avoir été énervant, la compétition est\n\
plus rude que jamais !"))

DEFINE_DIALOG(DIALOG_QUIZ_28, 1, 6, 95, 180, _("\
Whomp Ouvrier, la prochaine question est pour vous.\n\
Qu'est-ce que ça vous fait d'être dans la même\n\
pièce que ce monstre inhumain ?"))

//whomp
DEFINE_DIALOG(DIALOG_QUIZ_29, 1, 6, 95, 180, _("\
C'est... difficile. Il a causé tant de peine, à moi et\n\
à mon peuple. Mais je ne veux pas perpétuer le cycle de\n\
la violence. Je refuse d'antagoniser Mario."))

//sr
DEFINE_DIALOG(DIALOG_QUIZ_30, 1, 6, 95, 180, _("\
J'en pleurerais presque. Je vous accorde deux points\n\
pour cette réponse, Whomp Ouvrier. Tout le monde\n\
devrait prendre exemple sur vous."))

DEFINE_DIALOG(DIALOG_QUIZ_31, 1, 6, 95, 180, _("\
La tension monte sur le plateau... Whomp Ouvrier n'est\n\
plus qu'à un point de la victoire ! À présent, une\n\
question pour notre plus petit candidat (moralement)."))

DEFINE_DIALOG(DIALOG_QUIZ_32, 1, 6, 95, 180, _("\
Ressens-tu le moindre regret ?"))

//mario
DEFINE_DIALOG(DIALOG_QUIZ_33, 1, 6, 95, 180, _("\
A) Non B) Non\n\
C) Non D) Non"))

//whomp (stop music)
DEFINE_DIALOG(DIALOG_QUIZ_34, 1, 6, 95, 180, _("\
Ça me fait mal. Pas les choses que tu as déjà faites,\n\
mais le fait que tu n'arrives pas à reconnaître tes torts."))

DEFINE_DIALOG(DIALOG_QUIZ_35, 1, 6, 95, 180, _("\
Excepté les thwomp, tout le monde peut changer, et\n\
tout le monde mérite de pouvoir vivre une vie meilleure.\n\
Tout ce que tu dois faire, c'est accepter tes erreurs."))

DEFINE_DIALOG(DIALOG_QUIZ_36, 1, 6, 95, 180, _("\
Si tu peux faire ça, pour moi, pour mon peuple...\n\
Je pourrais peut-être te pardonner, Mario."))

//sr
DEFINE_DIALOG(DIALOG_QUIZ_37, 1, 6, 95, 180, _("\
Troisième question pour notre ami Whomp.\n\
Si j'ajoute cent pièces de plus à la cagnotte,\n\
pour ou contre une baston avec Mario ?"))

//whomp
DEFINE_DIALOG(DIALOG_QUIZ_38, 1, 6, 95, 180, _("\
POUR !! 100% POUR !! ALLEZ VIENS LÀ, MINABLE !!\n\
JE VAIS TE RÉDUIRE EN POUSSIÈRE, TU VAS PAYER\n\
POUR CE QUE TU AS FAIT !!"))

//sr
DEFINE_DIALOG(DIALOG_QUIZ_39, 1, 6, 95, 180, _("\
Quelle étonnant retournement de situation !\n\
Notre révolutionnaire de pierre va-t-il triompher\n\
du tyran rouge de sang ? J'en doute, mais regardons !"))

DEFINE_DIALOG(DIALOG_QUIZ_40, 1, 6, 95, 180, _("\
Whomp Ouvrier, vous serez assisté par quelques\n\
phantasmes cosmiques. Bonne chance !\n\
...Et mario, pareil. Allez allez."))

//fight

//sr
DEFINE_DIALOG(DIALOG_QUIZ_41, 1, 6, 95, 180, _("\
Aïe aïe aïe... On dirait bien que Mario ressort\n\
victorieux de ce combat ! Quel dommage..."))

DEFINE_DIALOG(DIALOG_QUIZ_42, 1, 6, 95, 180, _("\
Eh bien, Whomp Ouvrier ?\n\
Un dernier mot pour les caméras ?"))

//whomp
DEFINE_DIALOG(DIALOG_QUIZ_43, 1, 6, 95, 180, _("\
...Ce n'est pas grave. Rien n'est grave.\n\
Le résultat de ce combat n'a aucune importance."))

DEFINE_DIALOG(DIALOG_QUIZ_44, 1, 6, 95, 180, _("\
Tout ce qui compte, c'est de croire en quelque chose.\n\
De se battre pour quelque chose, de périr pour\n\
quelque chose."))

DEFINE_DIALOG(DIALOG_QUIZ_45, 1, 6, 95, 180, _("\
J'ai été vaincu, mais ma vie ?\n\
J'en suis entièrement satisfait."))

DEFINE_DIALOG(DIALOG_QUIZ_46, 1, 6, 95, 180, _("\
Les Whomps reconstruiront, j'en suis convaincu.\n\
Parce que l'on est forts. Parce que l'on a dû\n\
faire des sacrifices."))

DEFINE_DIALOG(DIALOG_QUIZ_47, 1, 6, 95, 180, _("\
Tu ne sais pas ce qu'est un sacrifice, Mario.\n\
Une fois tout ça terminé, tu retourneras courir et\n\
sauter dans ton petit monde horriblement parfait."))

DEFINE_DIALOG(DIALOG_QUIZ_48, 1, 6, 95, 180, _("\
Tu te fiches bien du mal que tu fais. Tu te fiches du\n\
chaos que tu laisses derrière toi. Tu n'as même pas\n\
besoin de penser à tout ça."))

DEFINE_DIALOG(DIALOG_QUIZ_49, 1, 6, 95, 180, _("\
Malgré tout, c'est moi qui ai gagné. Tu n'as pas pu\n\
détruire mon esprit, parce que tu n'en a pas toi-même.\n\
Tu mourras pour rien, parce que tu ne crois en rien."))

DEFINE_DIALOG(DIALOG_QUIZ_50, 1, 6, 95, 180, _("\
On se retrouvera en enfer, espèce de monstre."))

//sr
DEFINE_DIALOG(DIALOG_QUIZ_51, 1, 6, 95, 180, _("\
...Ah là là ! Les élucubrations d'un homme qui\n\
a tout sacrifié. Qu'est-ce que c'est poétique !"))

DEFINE_DIALOG(DIALOG_QUIZ_52, 1, 6, 95, 180, _("\
...Malgré tout, j'ai l'impression qu'il y a\n\
un fond de vérité à ses paroles."))

DEFINE_DIALOG(DIALOG_QUIZ_53, 1, 6, 95, 180, _("\
Mais pourquoi te bats-tu? Pourquoi fais-tu\n\
tout ça ? Quelles sont tes motivations ?"))

DEFINE_DIALOG(DIALOG_QUIZ_54, 1, 6, 95, 180, _("\
A) Je veux retrouver ma liberté.\n\
B) Je veux libérer les mondes."))

DEFINE_DIALOG(DIALOG_QUIZ_55_A, 1, 6, 95, 180, _("\
Oh. Alors, toute la destruction que tu sèmes ici,\n\
c'est simplement pour retrouver ta liberté...?"))

DEFINE_DIALOG(DIALOG_QUIZ_56_A, 1, 6, 95, 180, _("\
Agamemnon et toi vous ressemblez plus qu'il n'y parait.\n\
Le statu quo est tellement important pour toi que tu\n\
te bats pour le préserver à tout prix."))

DEFINE_DIALOG(DIALOG_QUIZ_57_A, 1, 6, 95, 180, _("\
C'est pas ça, être un héros.\n\
Et ça ne le sera jamais."))

DEFINE_DIALOG(DIALOG_QUIZ_55_B, 1, 6, 95, 180, _("\
Donc tu as bien des convictions, au final.\n\
C'est pour ça que tu voles nos Graines Cosmiques..."))

DEFINE_DIALOG(DIALOG_QUIZ_56_B, 1, 6, 95, 180, _("\
Je ne peux pas te soutenir dans cette mission, mais\n\
je pensais que tu faisais ça pour ton bien personnel.\n\
Ce n'est pas l'idée que je me faisais de toi."))

DEFINE_DIALOG(DIALOG_QUIZ_57, 1, 6, 95, 180, _("\
Voilà une question un peu plus personnelle.\n\
Comment me décrirais-tu ?"))

DEFINE_DIALOG(DIALOG_QUIZ_57_Q, 1, 6, 95, 180, _("\
A) Un méchant.\n\
B) Un rouage dans la machine."))

DEFINE_DIALOG(DIALOG_QUIZ_58_A, 1, 6, 95, 180, _("\
Pourquoi est-ce que c'est toujours noir ou blanc pour toi ?\n\
Tu ne vois jamais de gris, peu importe les circonstances..."))

DEFINE_DIALOG(DIALOG_QUIZ_59_A, 1, 6, 95, 180, _("\
Je suis déçu que tu n'arrives pas à saisir la moindre\n\
forme de nuance... Peut-être que tu comprendras quand tu\n\
auras une Graine Cosmique plantée dans le crâne. Peut-être."))

DEFINE_DIALOG(DIALOG_QUIZ_58_B, 1, 6, 95, 180, _("\
Hmm. Mon but, c'est l'art. Le divertissement.\n\
Ça l'a toujours été. C'est ça, la ''fin'' que je\n\
cherche sans cesse à atteindre."))

DEFINE_DIALOG(DIALOG_QUIZ_59_B, 1, 6, 95, 180, _("\
Mais il semblerait que ce soit les ''moyens''\n\
qui ont marqué les esprits. Ça ne me plaît pas."))

DEFINE_DIALOG(DIALOG_QUIZ_60_B, 1, 6, 95, 180, _("\
Dans tous les cas, ça n'a pas d'importance.\n\
On est pratiquement condamnés à être ennemis,\n\
au point où on en est. Telle est la nature des choses."))

DEFINE_DIALOG(DIALOG_QUIZ_61, 1, 6, 95, 180, _("\
Dernière question. Le Directeur, La Lune Parasite,\n\
Agamemnon... Peu importe le nom que tu lui donnes.\n\
Qu'en penses-tu ?"))

DEFINE_DIALOG(DIALOG_QUIZ_61_Q, 1, 6, 95, 180, _("\
A) Je ne lui ferai rien. B) Je veux la détruire.\n\
C) Je ne sais pas encore."))

//-1
DEFINE_DIALOG(DIALOG_QUIZ_62_A, 1, 6, 95, 180, _("\
Alors, tu cherches à me détruire, puis fuir cet endroit..."))

DEFINE_DIALOG(DIALOG_QUIZ_63_A, 1, 6, 95, 180, _("\
Tu n'as aucun plan. Tu vas condamner ce monde.\n\
J'en suis persuadé."))

//+1
DEFINE_DIALOG(DIALOG_QUIZ_62_B, 1, 6, 95, 180, _("\
Ah... On ne peut pas vraiment 'détruire' Agamemnon.\n\
C'est un corps céleste. Mais il est sans doute possible\n\
de la faire entrer en collision avec la Terre..."))

DEFINE_DIALOG(DIALOG_QUIZ_63_B, 1, 6, 95, 180, _("\
Pas que ça ait grande importance.\n\
Je t'en empêcherai avant même que tu en aies\n\
l'opportunité. Alors n'y compte pas trop."))

//+0
DEFINE_DIALOG(DIALOG_QUIZ_62_C, 1, 6, 95, 180, _("\
Pris dans le feu de l'action, tu n'as même pas pensé\n\
à lever les yeux vers le ciel étoilé."))

DEFINE_DIALOG(DIALOG_QUIZ_63_C, 1, 6, 95, 180, _("\
Si c'est vraiment le cas, tu ferais mieux de me livrer\n\
les Graines Cosmiques tout de suite..."))

//
DEFINE_DIALOG(DIALOG_QUIZ_64_A, 1, 6, 95, 180, _("\
J'espère que tu as bien compris que tu ne vaux pas\n\
mieux que les êtres minables qui peuplent les\n\
mondes des tableaux."))

DEFINE_DIALOG(DIALOG_QUIZ_65_A, 1, 6, 95, 180, _("\
Tu es tout aussi égoïste, tout aussi naïf, et tout aussi\n\
aveugle qu'eux aux réels problèmes que court ce monde."))

DEFINE_DIALOG(DIALOG_QUIZ_66_A, 1, 6, 95, 180, _("\
Si tu es ce que la Terre a de mieux à offrir,\n\
alors elle mérite d'être assimilée."))

DEFINE_DIALOG(DIALOG_QUIZ_67_A, 1, 6, 95, 180, _("\
Prends la clé, prends ta récompense.\n\
Fais un tour au cinéma quand tu seras prêt à mourir."))

DEFINE_DIALOG(DIALOG_QUIZ_64_B, 1, 6, 95, 180, _("\
C'est la fin de mon petit interrogatoire.\n\
Il semblerait que mon intuition était erronée.\n\
Tu as bel et bien des convictions."))

DEFINE_DIALOG(DIALOG_QUIZ_65_B, 1, 6, 95, 180, _("\
J'ai beau avoir de la peine pour ta cause, ce n'est pas\n\
mon genre de retourner ma veste. Et de toutes façons, je\n\
ne peux rien faire avec cette Graine Cosmique en moi."))

DEFINE_DIALOG(DIALOG_QUIZ_66_B, 1, 6, 95, 180, _("\
On finira probablement par s'entretuer. Rigolo, non ?\n\
Au moins, ça me rassure de savoir que le scénario\n\
dans lequel je perds ne sera pas le pire de tous."))

DEFINE_DIALOG(DIALOG_QUIZ_67_B, 1, 6, 95, 180, _("\
Tu as gagné le quiz. Bravo ! Voilà ta récompense,\n\
plombier. Cette clé t'emmènera directement au cinéma où\n\
sera projetée l'avant-première de 'Mario vs Urbowser'."))

DEFINE_DIALOG(DIALOG_QUIZ_68_B, 1, 6, 95, 180, _("\
Passe y faire un tour ! Et quand tu seras prêt,\n\
viens me voir sous le dôme. On règlera tout ça."))

DEFINE_DIALOG(DIALOG_QUIZ_69_B, 1, 6, 95, 180, _("\
Fin du spectacle."))

DEFINE_DIALOG(DIALOG_SB_1, 1, 6, 95, 180, _("\
Mario ! Mario. Mario ! Salut, superstar ! Que penses-tu\n\
de Festella ? C'est tout ce que j'imaginais, et bien\n\
plus encore. Il y a de la joie à tous les coins de rue !"))

DEFINE_DIALOG(DIALOG_SB_2, 1, 6, 95, 180, _("\
Voir les habitants sourire emplit\n\
de bonheur ma pauvre âme corrompue."))

DEFINE_DIALOG(DIALOG_SB_3, 1, 6, 95, 180, _("\
Mario VS Urbowser marche du tonnerre, tout le monde en\n\
parle ! Le Directeur aurait adoré, mais son avatar a\n\
mystérieusement disparu. Mais bon. Peu importe."))                                 //"no more delays then"?//

DEFINE_DIALOG(DIALOG_SB_4, 1, 6, 95, 180, _("\
La planète pourrait tenir encore quelques décennies avant\n\
d'être absorbée. Dommage que tu sois si opposé à l'idée\n\
d'offrir une vie digne à ses habitants avec les Graines."))

DEFINE_DIALOG(DIALOG_SB_5, 1, 6, 95, 180, _("\
Quand j'ai accepté le rôle de prophète, je me suis\n\
fait à l'idée de notre inévitable mortalité.\n\
Et toi ?"))

DEFINE_DIALOG(DIALOG_SB_6, 1, 6, 95, 180, _("\
...Tu ne pourras pas l'arrêter, tu sais. Il est impossible\n\
de détruire un corps céleste. On ne peut pas négocier non\n\
plus. C'est un processus naturel, et on est pris dedans."))

DEFINE_DIALOG(DIALOG_SB_7, 1, 6, 95, 180, _("\
J'ai décidé de me laisser emporter par la vague.\n\
Pourquoi ne pas nager avec moi ?"))

DEFINE_DIALOG(DIALOG_SB_8, 1, 6, 95, 180, _("\
Ça n'a plus grande importance, de toutes façons. Ce qui\n\
est fait est fait. Je doute pouvoir te convaincre.\n\
Dans ce cas, passons à la grande confrontation !"))

DEFINE_DIALOG(DIALOG_SB_9, 1, 6, 95, 180, _("\
Lequel de nous deux aura sa happy end ?"))

DEFINE_DIALOG(DIALOG_SB_1B, 1, 6, 95, 180, _("\
...Eeeet... Coupez.\n\
Bien joué, très bien joué."))

DEFINE_DIALOG(DIALOG_SB_2B, 1, 6, 95, 180, _("\
...Alors, c'est comme ça que tout se termine ?\n\
Mon histoire prend fin le jour où sort la tienne ?\n\
Quelle ironie."))

DEFINE_DIALOG(DIALOG_SB_3B, 1, 6, 95, 180, _("\
..."))

DEFINE_DIALOG(DIALOG_SB_4B, 1, 6, 95, 180, _("\
Ouais, ouais. Je savais bien que ça allait se terminer.\n\
Mais le spectacle... Il était bien, non ? On s'est bien\n\
amusés. Dommage qu'on ait dû être l'un contre l'autre."))

DEFINE_DIALOG(DIALOG_SB_5B, 1, 6, 95, 180, _("\
J'espère que je ne périrai pas en vain. \n\
Va détruire la lune, Mario. Peu importe les conséquences.\n\
Fais ce que tu as à faire."))

DEFINE_DIALOG(DIALOG_SB_6B, 1, 6, 95, 180, _("\
Je te lègue mon projecteur. Il établira\n\
un pont de lumière jusqu'à Agamemnon."))

DEFINE_DIALOG(DIALOG_SB_7B, 1, 6, 95, 180, _("\
Cependant, si tu décides d'y aller, sache que\n\
tu n'as pas la moindre chance d'y survivre.\n\
Et n'imagine même pas revenir sur Terre."))

DEFINE_DIALOG(DIALOG_SB_8B, 1, 6, 95, 180, _("\
...Bon, j'aurai bien rigolé. Être une star, c'est\n\
enivrant... mais je m'amuserai encore plus au firmament.\n\
...On se retrouvera là-bas."))

DEFINE_DIALOG(DIALOG_CREDITS_1, 1, 6, 95, 180, _("\
- Rovertronic -\n\
Développeur principal"))

DEFINE_DIALOG(DIALOG_CREDITS_2, 1, 6, 95, 180, _("\
- RationaLess -\n\
Direction créative, Écriture, Modèles 3D"))

DEFINE_DIALOG(DIALOG_CREDITS_3, 1, 6, 95, 180, _("\
- Thorndust, SpK, & ArcticJaguar725 -\n\
Composition et arrangement de la bande-son\n\
originale de SM64: Beyond the Cursed Mirror"))

DEFINE_DIALOG(DIALOG_CREDITS_4, 1, 6, 95, 180, _("\
- axollyon -\n\
Portage vers HackerSM64, Code des skybox"))

DEFINE_DIALOG(DIALOG_CREDITS_5, 1, 6, 95, 180, _("\
- BoomerDaCat -\n\
Peintures du château, Cover Art"))                                               // ???

DEFINE_DIALOG(DIALOG_CREDITS_6, 1, 6, 95, 180, _("\
- biobak -\n\
Écran titre, Végétation\n\
Traduction et localisation"))                                                   //hehe

DEFINE_DIALOG(DIALOG_CREDITS_7, 1, 6, 95, 180, _("\
- Anonymous Moose -\n\
Better Extended Bounds, Texture de métal\n\
Création de la Reine Thwomp"))

DEFINE_DIALOG(DIALOG_CREDITS_8, 1, 6, 95, 180, _("\
- Someone2639 -\n\
Patch Shiftable Segments"))

DEFINE_DIALOG(DIALOG_CREDITS_9, 1, 6, 95, 180, _("\
- CrashOverride -\n\
Aide à l'implémentation des vibrations"))

DEFINE_DIALOG(DIALOG_CREDITS_10, 1, 6, 95, 180, _("\
- HeroTechne -\n\
Visuels des badges (12 premiers)"))

DEFINE_DIALOG(DIALOG_CREDITS_11, 1, 6, 95, 180, _("\
- Pyro Jay -\n\
Visuels des badges (12 suivants),\n\
Amélioration des textures"))

DEFINE_DIALOG(DIALOG_CREDITS_12, 1, 6, 95, 180, _("\
- SpK -\n\
Don du Crâne de Spooky Mario 666 pour le \n\
costume de Pirate, Aide avec la musique"))

DEFINE_DIALOG(DIALOG_CREDITS_13, 1, 6, 95, 180, _("\
- aWiseguyEh -\n\
Calculs World Space vers Screen Space\n\
Modèle de Motos"))

DEFINE_DIALOG(DIALOG_CREDITS_14, 1, 6, 95, 180, _("\
- Arthurtilly -\n\
Platform Displacement 2, Code de Motos, Aide avec\n\
Bad Apple, Moteur de simulation de physique"))

DEFINE_DIALOG(DIALOG_CREDITS_15, 1, 6, 95, 180, _("\
- Reonu -\n\
Mode 16:9 sur N64, Warp Floors"))

DEFINE_DIALOG(DIALOG_CREDITS_16, 1, 6, 95, 180, _("\
- Falcobuster -\n\
World Precision Fix"))

DEFINE_DIALOG(DIALOG_CREDITS_17, 1, 6, 95, 180, _("\
- Yoshi Milkman -\n\
Modèle du Rex de Super Mario World"))

DEFINE_DIALOG(DIALOG_CREDITS_18, 1, 6, 95, 180, _("\
- theCozies -\n\
Spline Tool, N64 Float Denorm Fixes"))

DEFINE_DIALOG(DIALOG_CREDITS_19, 1, 6, 95, 180, _("\
- CowQuack -\n\
Aide avec EnvFx, Code de changement de tempo"))

DEFINE_DIALOG(DIALOG_CREDITS_20, 1, 6, 95, 180, _("\
- Furyiousfight -\n\
Textures de la ville"))

DEFINE_DIALOG(DIALOG_CREDITS_21, 1, 6, 95, 180, _("\
- Xinus22 -\n\
Textures de Xen de HL1, Modèle de Maxwell"))

DEFINE_DIALOG(DIALOG_CREDITS_22, 1, 6, 95, 180, _("\
- Blakeoramo -\n\
Optimisation de la collision du monde 1"))

DEFINE_DIALOG(DIALOG_CREDITS_22_A, 1, 6, 95, 180, _("\
- aglab2 -\n\
Correction d'un bug avec les ombres des boîtes"))

DEFINE_DIALOG(DIALOG_CREDITS_23, 1, 6, 95, 180, _("\
- InspectorJ, Day Garwood, ngruber -\n\
Effets sonores de pluie, de foule et de\n\
verre brisé, via freesound.org"))

DEFINE_DIALOG(DIALOG_CREDITS_24, 1, 6, 95, 180, _("\
Bêta Tests: Phase 0 - Hyena Chan, BoomerDaCat\n\
Phase 1 - CowQuack, Fazana, GTM, ShiN3, Dan GPTV\n\
Phase 2 - Spk, ArcticJaguar725"))

DEFINE_DIALOG(DIALOG_CREDITS_25, 1, 6, 95, 180, _("\
- Drahnokks, Indigo Dindigo -\n\
Importation des lettres accentuées\n\
Remplacement de certaines textures"))
//This special thanks screen sucks. I'm going to be removing it
//until I have something more meaningful and not shoehorned in
//it also needs to be more tasteful

DEFINE_DIALOG(DIALOG_CREDITS_26, 1, 6, 95, 180, _("\
Patreons au 27¤/09¤/2023:\n\
Henry Kumpel, ZT, Pizza Brian, tydaze"))

DEFINE_DIALOG(DIALOG_PALAMEDES_1, 1, 6, 95, 180, _("\
La synthétisation de graines nécessite l'attention constante\n\
des sujets. Elle est recueillie par les hérauts. Le noyau de\n\
la Terre va être absorbé. Votre monde est Agamemnon."))

DEFINE_DIALOG(DIALOG_POEM_1, 1, 6, 95, 180, _("\
Quand le monde vivait de bonheur abondant,\n\
Quand le monstre fut forcé à se limer les dents,\n\
Quand les pécheurs laissèrent place aux vertueux ;"))

DEFINE_DIALOG(DIALOG_POEM_2, 1, 6, 95, 180, _("\
Quand les Hommes aspirèrent à proclamer les cieux,\n\
Une ombre grandissante, une menace implicite\n\
La fin de toute chose, la lune parasite "))

DEFINE_DIALOG(DIALOG_POSTGAME, 1, 6, 95, 180, _("\
Bienvenue dans l'après-jeu ! Tout ce qui se passe\n\
à partir de maintenant ne fait plus partie de\n\
l'histoire du jeu."))

//LCOME TO THE HUSHED

DEFINE_DIALOG(DIALOG_GIRL_1A, 1, 4, 95, 180, _("\
Coucou !!!! Oh, c'est toi,\n\
Mario Mario !! :3c\n\
Toi aussi, t'es une\n\
star du showbiz !!\n\
On est en tournée à Festella\n\
pour profiter de tout ce \n\
monde qui vient voir ton\n\
super film !!\n\
Malheureusement, mes soeurs\n\
ont l'air d'avoir oublié\n\
qu'on devait se représenter\n\
sur scène bientôt...\n\
Éblouies par les lumières\n\
de la ville, elles se sont\n\
perdues dans la nuit !\n\
\n\
Tu pourrais trouver mes\n\
quatre soeurs et leur dire\n\
que le concert va bientôt\n\
commencer, onegaishimasu ?\n\
Juri est passionnée\n\
de logistique...\n\
Je sais pas exactement\n\
ce que ça implique...\n\
Mais c'est le seul\n\
indice que j'ai :3\n\
Je les attends ici !\n\
Arigato, Mario !"))

DEFINE_DIALOG(DIALOG_GIRL_1B, 1, 2, 95, 180, _("\
La nouvelle lune me\n\
met mal à l'aise...\n\
Pourquoi est-ce qu'elle\n\
est si proche de Festella ?"))

DEFINE_DIALOG(DIALOG_GIRL_2A, 1, 3, 95, 180, _("\
Tu veux quoi ?\n\
T'es louche.\n\
...T'es un flic ?\n\
Oooh, attends, attends.\n\
T'es le type du film\n\
au cinéma, là...\n\
...Bowser ! Ça va, mec ?\n\
C'est comment, la vie\n\
de star ?\n\
...Yuri nous attend\n\
pour le concert ?\n\
Argh, okayyyyyy...\n\
Si tu cherches Aoi,\n\
elle doit être sur\n\
la grande roue.\n\
Elle arrêtait pas d'en\n\
parler dans le bus !\n\
\n\
Moi, je suis un peu\n\
occupée avec des\n\
TRUCS de RICHE, là.\n\
Merci de m'avoir informée,\n\
mais je n'abandonne\n\
jamais mes projets."))

DEFINE_DIALOG(DIALOG_GIRL_2B, 1, 4, 95, 180, _("\
J'y vais bientôt...\n\
Je vais juste... admirer\n\
ce container... encore\n\
un petit peu..."))

DEFINE_DIALOG(DIALOG_GIRL_3A, 1, 7, 95, 180, _("\
''Étoile''... quel joli\n\
mot ! Il y a les étoiles\n\
dans le ciel, les étoiles\n\
de puissance, et bien sûr\n\
les étoiles dans les yeux\n\
de mes fans quand ils me\n\
voient sur scène !\n\
Elles sont toutes si\n\
surréelles, hors de portée\n\
du commun des mortels tant\n\
elles sont mythiques.\n\
C'est ce côté mythologique\n\
qui attise les esprits et\n\
qui les rend mystérieuses.\n\
On a tous besoin de leur\n\
scintillement dans nos\n\
vies si ternes ! Enfin,\n\
celle des autres. Ma vie\n\
est déjà merveilleuse.\n\
Mais leur existence pose\n\
une autre question.\n\
Elles emplissent nos\n\
vies de lumière, mais\n\
pourquoi avons-nous même\n\
besoin de lumière ?\n\
Pour contraster les\n\
ténèbres infinies du cosmos ?\n\
On ne saura sûrement jamais.\n\
Oh ! Je déblatère encore.\n\
Je vais bientôt aller\n\
retrouver mes soeurs.\n\
Je veux juste regarder\n\
le ciel étoilé encore\n\
un petit peu...\n\
\n\
...Tu cherches toujours\n\
Furi ? Elle est sans\n\
doute dans les rues de\n\
la ville. Elle adore les\n\
lumières ! Et puis, les\n\
néons, c'est un peu comme\n\
des étoiles."))

DEFINE_DIALOG(DIALOG_GIRL_3B, 1, 2, 95, 180, _("\
Quand je serai morte, je\n\
veux devenir une étoile."))

DEFINE_DIALOG(DIALOG_GIRL_4A, 1, 3, 95, 180, _("\
...Il faut en faire tant\n\
pour ne pas être oublié...\n\
C'est épuisant.\n\
Tout ce que je veux, c'est\n\
la paix et le calme, mais\n\
c'est si compliqué...\n\
Ah ! Toutes mes excuses\n\
je doute que tu veuilles\n\
m'écouter me lamenter.\n\
Tu ne veux qu'avancer.\n\
Je te dirais bien que\n\
c'est admirable, mais\n\
ce n'est que le cours\n\
normal des choses.\n\
\n\
Je m'en irai bientôt.\n\
Merci."))

DEFINE_DIALOG(DIALOG_GIRL_4B, 1, 1, 95, 180, _("\
..."))

DEFINE_DIALOG(DIALOG_SHOWRUNNER_BAD_1, 1, 1, 95, 180, _("\
..."))

DEFINE_DIALOG(DIALOG_SHOWRUNNER_BAD_2, 1, 1, 95, 180, _("\
...va crever."))

DEFINE_DIALOG(DIALOG_EXEC_1_A, 1, 1, 0, 0, _("\
ÉCOUTE, ACTEUR. TU ES CONSCIENT."))

DEFINE_DIALOG(DIALOG_EXEC_2_A, 1, 1, 0, 0, _("\
JE SUIS LE DIRECTEUR. JE SUPERVISE LE STUDIO\n\
SUR TERRA. NOUS SOMMES TOUS DEUX EUKARYOTES.\n\
EN TERMES FAMILIERS, JE SUIS LE RÉTICULE."))

DEFINE_DIALOG(DIALOG_EXEC_3_A, 1, 1, 0, 0, _("\
MON AVATAR N'A PAS PU SE MANIFESTER SOUS FORME\n\
PHYSIQUE POUR CAUSE D'INTERFÉRENCES PROVENANT\n\
D'UN OBSERVATOIRE. CELUI-CI EST DONC VIRTUEL."))

DEFINE_DIALOG(DIALOG_EXEC_4_A, 1, 1, 0, 0, _("\
TU NOUS A ÉTÉ D'UNE GRANDE AIDE. L'ATTENTION\n\
DES PORTEURS DE GRAINES A ÉTÉ EXCELLENTE.\n\
AGAMEMNON VA ENTRER EN PHASE DE PHAGOCYTOSE."))

DEFINE_DIALOG(DIALOG_EXEC_5_A, 1, 1, 0, 0, _("\
TU NE SEMBLES PAS VOULOIR DEVENIR UN PORTEUR\n\
DE GRAINE. IL EST POSSIBLE DE CONVAINCRE LES\n\
ÊTRES DOTÉS D'INTELLIGENCE."))

DEFINE_DIALOG(DIALOG_EXEC_6_A, 1, 1, 0, 0, _("\
TON PROPHÈTE ATTITRÉ(présentateur) T'A\n\
EXPOSÉ À DES VISIONS FAVORABLES."))

DEFINE_DIALOG(DIALOG_EXEC_7_A, 1, 1, 0, 0, _("\
POURTANT, TU REFUSES ENCORE D'ÊTRE CONVERTI.\n\
CETTE CONVERSATION VA DONC PRENDRE FIN."))



DEFINE_DIALOG(DIALOG_EXEC_1_B, 1, 1, 0, 0, _("\
PAUSE ! On se calme ! Pour l'amour de la lune,\n\
je te l'avais dit ! Je ne t'avais interdit\n\
qu'une seule chose, et c'était celle-là !"))

DEFINE_DIALOG(DIALOG_EXEC_2_B, 1, 1, 0, 0, _("\
Notre magnifique, fantastique, cylindrique directeur.\n\
Ma vie, non, ma CARRIÈRE toute entière dépend de lui.\n\
Et toi, tu veux le détruire, pourquoi exactement ?!"))

DEFINE_DIALOG(DIALOG_EXEC_3_B, 1, 1, 0, 0, _("\
Qu'est-ce qu'il se passe dans ta... ta...\n\
Dans ta tête phrénologiquement inadaptée ?"))

DEFINE_DIALOG(DIALOG_EXEC_4_B, 1, 1, 0, 0, _("\
Tu ne comprends rien. Tu ne comprends même pas à quel\n\
point tu ne comprends rien. Tout ce qui se passe ici\n\
s'y passe pour une bonne raison."))

DEFINE_DIALOG(DIALOG_EXEC_5_B, 1, 1, 0, 0, _("\
Tes actions vont nous envoyer directement dans la\n\
pire version possible de cet univers."))

DEFINE_DIALOG(DIALOG_EXEC_6_B, 1, 1, 0, 0, _("\
...Bon, eh bien... Tu sais comment ça marche.\n\
Le Prophète protège l'Avatar."))

DEFINE_DIALOG(DIALOG_EXEC_7_B, 1, 1, 0, 0, _("\
Tu sais, Mario...\n\
J'espérais que l'on n'en arrive pas là..."))

DEFINE_DIALOG(DIALOG_EXEC_8_B, 1, 1, 0, 0, _("\
...Non, j'ai menti. Depuis le début,\n\
je n'espérais que ça. En garde, machin rouge !\n\
Je vais rayer ton nom du casting !"))



DEFINE_DIALOG(DIALOG_EXEC_1_C, 1, 1, 0, 0, _("\
Quoi...? Qu'est-ce que...?"))

//<skip>

DEFINE_DIALOG(DIALOG_EXEC_2_C, 1, 1, 0, 0, _("\
Je ne peux même pas t'expliquer les conséquences\n\
interplanétaires de ce que tu viens de faire."))

DEFINE_DIALOG(DIALOG_EXEC_3_C, 1, 1, 0, 0, _("\
La terreur existentielle inimaginable que\n\
tu devrais être en train de ressentir.\n\
Je n'ai pas les mots."))

DEFINE_DIALOG(DIALOG_EXEC_4_C, 1, 1, 0, 0, _("\
Non, non. Tu sais quoi, tout va bien.\n\
On a qu'à acheter un autre directeur sur Amazon, et\n\
la lune ne nous avalera pas pendant encore 100 ans."))

DEFINE_DIALOG(DIALOG_EXEC_5_C, 1, 1, 0, 0, _("\
C'est totalement un truc qu'on peut faire.\n\
Tout va bien dans le meilleur des mondes !"))

DEFINE_DIALOG(DIALOG_EXEC_6_C, 1, 1, 0, 0, _("\
Tu es quelqu'un de super et de très agréable.\n\
Tu es très respectable. Tu fais plein de choses\n\
bien et de choses gentilles. Un type génial."))

DEFINE_DIALOG(DIALOG_EXEC_7_C, 1, 1, 0, 0, _("\
Tout le monde t'adore.\n\
Ta tête a une forme parfaitement normale !"))

DEFINE_DIALOG(DIALOG_EXEC_8_C, 1, 1, 0, 0, _("\
Félicitations Mario, tu existes.\n\
Je sais même pas par où commencer\n\
à réparer cette catastrophe."))

DEFINE_DIALOG(DIALOG_EXEC_9_C, 1, 1, 0, 0, _("\
Je m'occuperai de toi plus tard.\n\
J'en peux plus. Je me tire."))

DEFINE_DIALOG(DIALOG_MUSICROOM, 1, 5, 95, 180, _("\
Titre : [%]\n\
Par : [%]"))

DEFINE_DIALOG(DIALOG_LVUP, 1, 5, 95, 180, _("\
GAIN DE NIVEAU ! Mario est désormais niveau [%].\n\
Ses PV et son Mana maximum ont augmenté.\n\
Il peut également porter davantage de badges."))

DEFINE_DIALOG(DIALOG_SWITCH_1, 1, 5, 95, 180, _("\
Fais un triple saut pour t'envoler. \n\
Maintiens [B] pour te propulser, mais \n\
garde un oeil ta jauge d'essence !"))

DEFINE_DIALOG(DIALOG_SWITCH_2, 1, 5, 95, 180, _("\
Tu peux désormais trouver des graines cosmiques\n\
dans les boîtes noires. Sois prêt à affronter\n\
les horreurs qui les gardent."))

DEFINE_DIALOG(DIALOG_SWITCH_3, 1, 5, 95, 180, _("\
Cette casquette combine les pouvoirs d'invisibilité\n\
et de métal. Tu peux rebondir sur les surfaces\n\
brûlantes et traverser les murs les plus fins."))

DEFINE_DIALOG(DIALOG_SWITCH_4, 1, 5, 95, 180, _("\
Grimpe sur une carapace pour surfer sur l'eau\n\
ou escalader même les pentes les plus raides."))

DEFINE_DIALOG(DIALOG_COSTUME_HINT1, 1, 4, 95, 180, _("\
Tu cherches un costume ?\n\
Voici une énigme qui\n\
devrait te mettre sur\n\
la bonne voie :\n\
Les chutes de lave abritent\n\
Sur une île de granit\n\
Un costume des temps anciens\n\
Qui te va toujours très bien."))

DEFINE_DIALOG(DIALOG_COSTUME_HINT2, 1, 4, 95, 180, _("\
Tu cherches un costume ?\n\
Voici une énigme qui\n\
devrait te mettre sur\n\
la bonne voie :\n\
Dans les grottes d'azur\n\
En haut d'un bâtiment\n\
Se trouve la parure\n\
D'un type un peu dément."))

DEFINE_DIALOG(DIALOG_COSTUME_HINT3, 1, 4, 95, 180, _("\
Tu cherches un costume ?\n\
Voici une énigme qui\n\
devrait te mettre sur\n\
la bonne voie :\n\
Comment est-il possible\n\
De rater celui-ci ?\n\
Un frère vert se cache\n\
Dans une pièce près d'ici..."))

DEFINE_DIALOG(DIALOG_COSTUME_HINT4, 1, 4, 95, 180, _("\
Tu cherches un costume ?\n\
Voici une énigme qui\n\
devrait te mettre sur\n\
la bonne voie :\n\
C'est un Mario à l'envers,\n\
Il est avare et grincheux;\n\
Dans un réservoir en fer\n\
Emprunte le chemin 2."))

DEFINE_DIALOG(DIALOG_COSTUME_HINT5, 1, 4, 95, 180, _("\
Tu cherches un costume ?\n\
Voici une énigme qui\n\
devrait te mettre sur\n\
la bonne voie :\n\
Sous le feu des projecteurs\n\
Quatre questions sont posées\n\
Ton instinct d'explorateur\n\
T'aidera à le trouver."))

DEFINE_DIALOG(DIALOG_COSTUME_HINT6, 1, 4, 95, 180, _("\
Tu cherches un costume ?\n\
Voici une énigme qui\n\
devrait te mettre sur\n\
la bonne voie :\n\
Le marin a disparu,\n\
Mais son esprit reste là:\n\
À l'intérieur du navire,\n\
En-dessous et au-delà."))

DEFINE_DIALOG(DIALOG_COSTUME_HINT7, 1, 4, 95, 180, _("\
Tu cherches un costume ?\n\
Voici une énigme qui\n\
devrait te mettre sur\n\
la bonne voie :\n\
Dans un monde virtuel,\n\
Qui est loin d'être rétro,\n\
La routine habituelle,\n\
Il attend le métro."))

DEFINE_DIALOG(DIALOG_COSTUME_HINT8, 1, 4, 95, 180, _("\
Tu cherches un costume ?\n\
Voici une énigme qui\n\
devrait te mettre sur\n\
la bonne voie :\n\
Dans les plaines sans pesanteur\n\
Où l'on jette la semence,\n\
Le costume du facteur\n\
Est plus proche que tu ne penses."))

DEFINE_DIALOG(DIALOG_COSTUME_HINT9, 1, 4, 95, 180, _("\
Tu cherches un costume ?\n\
Voici une énigme qui\n\
devrait te mettre sur\n\
la bonne voie :\n\
La demeure des humains\n\
Cache un souvenir du passé;\n\
Là où se lavent les mains\n\
Une étoile s'est noyée."))

DEFINE_DIALOG(DIALOG_COSTUME_HINT10, 1, 4, 95, 180, _("\
Tu cherches un costume ?\n\
Voici une énigme qui\n\
devrait te mettre sur\n\
la bonne voie :\n\
À l'endroit où les pixels\n\
Sont l'exclusive unité,\n\
Il faut connaître les ficelles\n\
Du monde qui l'a inspiré."))

DEFINE_DIALOG(DIALOG_COSTUME_HINT11, 1, 4, 95, 180, _("\
Tu cherches un costume ?\n\
Voici une énigme qui\n\
devrait te mettre sur\n\
la bonne voie :\n\
Loin au-dessus des nuages,\n\
Dans le territoire du roi,\n\
L'emplacement d'un secret\n\
Est marqué par une croix."))

DEFINE_DIALOG(DIALOG_COSTUME_HINT12, 1, 4, 95, 180, _("\
Tu cherches un costume ?\n\
Voici une énigme qui\n\
devrait te mettre sur\n\
la bonne voie :\n\
Dans un bâtiment\n\
baigné de lumière dorée,\n\
t'attend un accoutrement\n\
réservé aux ouvriers."))

DEFINE_DIALOG(DIALOG_COSTUME_HINT13, 1, 4, 95, 180, _("\
Tu cherches un costume ?\n\
Voici une énigme qui\n\
devrait te mettre sur\n\
la bonne voie :\n\
Elle tourne, tourne,\n\
Sans s'arrêter;\n\
Et au coeur de ses rayons,\n\
Un visage familier."))

DEFINE_DIALOG(DIALOG_COSTUME_HINT14, 1, 4, 95, 180, _("\
Tu cherches un costume ?\n\
Voici une énigme qui\n\
devrait te mettre sur\n\
la bonne voie :\n\
Un reflet empoisonné\n\
De l'image de Mario;\n\
Dans cet endroit désordonné\n\
Se déroule le scénario."))

DEFINE_DIALOG(DIALOG_COSTUME_END, 1, 2, 95, 180, _("\
Tu as tout trouvé !\n\
Félicitations !\n\
Bon, eh bien...\n\
\n\
Si tu as besoin de moi...\n\
\n\
...Je suis là..."))

DEFINE_DIALOG(DIALOG_WALLET_HELP, 1, 4, 95, 180, _("\
Tu cherches les bourses ?\n\
Il y en a une dans chaque\n\
monde principal, du Bassin\n\
Bouillant à Festella.\n\
Si tu trouves les 12 \n\
bourses, les pièces que\n\
tu ramèneras des mondes\n\
seront doublées.\n\
Tu peux vérifier si tu as\n\
trouvé la bourse d'un\n\
monde sur l'écran de\n\
sélection d'étoile.\n\
Combiné avec le badge\n\
Avarice, les pièces\n\
que tu trouves seront\n\
quadruplées."))

DEFINE_DIALOG(DIALOG_WALLET_EVIL, 1, 3, 30, 200, _("\
Tu as trouvé une bourse. Ton nombre de\n\
pièces maximal est passé de [%] à [%] !\n\
Elle contenait également... rien :("))